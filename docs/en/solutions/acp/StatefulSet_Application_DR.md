...
1. **Organize Code Repositories**: Store the application's Kubernetes manifests (including PVC, StatefulSet, and Service templates) in Git. For example:
   ...
   metadata:
     ...
     namespace: <application-namespace>
   spec:
     replicas: 1 # This will be patched to 0 for the standby cluster
     selector:
       matchLabels:
         app: my-stateful-app
...
2. **ApplicationSet Synchronization**: Configure an ApplicationSet to distribute resources simultaneously to both the primary and standby clusters. Due to the **Active-Passive model**, during normal operations:
   - The instance `replicas` for the **primary cluster** is set to `N`.
   - The instance `replicas` for the **standby cluster** must be set to `0` using `patches` in kustomize (to avoid invalid connections and data corruption when failover has not occurred).
...
> **Prerequisite**: Both primary and standby clusters must have `Alauda Build of VolSync` installed, alongside corresponding CSI storage drivers that support snapshot functionality.

**1. Prepare Trust Credentials Secret**

Create a uniform `rsync-tls` Secret in both the primary and standby clusters, defining the PSK for secure identity authentication.
...
Extract the exposed address of the standby cluster generated above, and instantiate the corresponding `ReplicationSource` in the namespace where the primary business application resides. Define a Cron expression in `trigger.schedule` (e.g., `*/10 * * * *` indicates a snapshot-level incremental sync every 10 minutes) to ensure business changes are periodically persisted to the standby cluster.
...
**Applicable Context**: Both primary and standby clusters are healthy; an active-standby inversion is executed smoothly due to datacenter migration or scheduled cutover.
...
2. **Execute Final Full Synchronization**:
   ...
   - Create a One-Time `ReplicationSource` with the `trigger.manual` label on the primary cluster. This ensures that any tail data generated just as the primary cluster shut down is transmitted to the standby cluster.
3. **Sever Synchronization Link**:
   - Once the final sync is complete, delete the newly created One-Time `ReplicationSource` and the `ReplicationDestination` on the standby cluster.
   - This signifies that the data on the standby cluster is now a complete mirror of the primary disk and is independently decoupled.
4. **Start Standby Cluster Services**: Through GitOps, restore the `replicas` on the standby cluster to its normal quantity. The business Pods will fully boot up on the standby cluster, mounting the synchronized PVC.
5. **Switch Network Traffic**: Manipulate the global GSLB or DNS to direct all external request ingress points to the standby cluster.
6. **Establish Reverse Backup Chain**: Create a periodic `ReplicationSource` on the newly active standby cluster, and transition the original primary cluster into a `ReplicationDestination`. The smooth active-standby inversion is complete.
...
2. **Rebuild Business Storage (PVC Restore)**:
   Because the primary cluster is down, the data last received by the standby cluster becomes the sole source of truth (subject to an RPO dependent on the synchronization frequency). Within the standby cluster, use a `dataSourceRef` to rebuild the persistent volume PVC from the most recent data:
...
3. **(Optional) Data Validation**: Launch an ephemeral Local Copy Sync source on the standby cluster utilizing the recovered `restored-pvc-01` to verify that the reconstructed data structure is secure and uncorrupted.
...
5. **Boot Standby Cluster Business Modules**: Force update the `replicas` of the target module on the standby cluster to the anticipated number via GitOps, and modify the volume declaration of the StatefulSet to mount the newly generated `restored-pvc-01` storage claim. The business will automatically restore.
6. **Traffic Failover**: Adjust DNS/GSLB, reconfigure the standby cluster's weight to the maximum, and take over traffic.
...
2. **Pull the Latest Data from the Standby Cluster**:
   ...
   - Manually trigger a One-Time `ReplicationSource` on the currently operational standby cluster (which is actively serving traffic) to push the newly accumulated operational data back to the primary cluster.
...
4. **Subsequent steps** are identical to `3.1 Planned Migration`: halt traffic to the standby cluster, scale up application replicas on the primary cluster, and resume operations.

...
The first should return `yes`, the second `no` (Deployments are in `apps`, not in the core group). If the second returns `yes`, the binding is broader than intended.

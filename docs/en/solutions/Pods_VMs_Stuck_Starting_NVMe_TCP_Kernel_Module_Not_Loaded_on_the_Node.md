...
Running `kubectl debug node/$NODE --image=busybox -- chroot /host lsmod | grep nvme_tcp` returns empty. The kernel is perfectly capable of running the NVMe-TCP stack — the module just has not been loaded, and the node is not configured to load it at boot.
...
```bash
NODE=<node>
kubectl debug node/$NODE --image=busybox -- chroot /host sh -c '
...
```bash
kubectl debug node/$NODE --image=busybox -- chroot /host modprobe nvme_tcp
```

...
# Collecting Performance Metrics With `sar` and `iostat` on Minimal-OS Nodes
## Issue
...
```bash
NODE=<node-name>
# Pick an image that ships sysstat AND is reachable from your cluster.
# Public images may not be pullable on isolated ACP clusters; mirror or
# substitute with the same image from your in-cluster registry.
IMAGE=<your-registry>/sysstat-toolbox:latest   # must include sar, iostat

# Note: chroot /host is rejected by the cluster-level Pod Security
# Admission policy on ACP, so the article does NOT chroot. The sysstat
# tools read /proc and /sys, which the debug pod sees through its own
# /proc/sys mount thanks to hostPID/hostNetwork set by `kubectl debug`.
kubectl debug node/$NODE -it --image=$IMAGE --profile=sysadmin -- sh -c '
...
`--profile=sysadmin` enables the host-namespace + host-PID flags that sysstat needs to read the right `/proc` entries. Without it the tools see the debug pod's own (empty) namespace.
...
```bash
NODE=<node-name>
IMAGE=<your-registry>/sysstat-toolbox:latest

# Output goes to /host/var/log/perf-capture (the host's /var/log via the
# debug pod's hostPath mount). chroot is not used — paths are written
# explicitly with the /host prefix.
kubectl debug node/$NODE -it --image=$IMAGE --profile=sysadmin -- sh -c '
  mkdir -p /host/var/log/perf-capture
  nohup sar -A -o /host/var/log/perf-capture/sar-$(date -u +%Y%m%dT%H%M%SZ).dat 10 360 >/dev/null 2>&1 &
...
kubectl debug node/$NODE -it --image=$IMAGE --profile=sysadmin -- \
  ls -lh /host/var/log/perf-capture/
```

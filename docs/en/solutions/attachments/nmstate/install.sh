...
# Image variables - modify these as needed
NMSTATE_REGISTRY="${NMSTATE_REGISTRY:-build-harbor.alauda.cn}"

echo "Installing NMState with the following registry:"
echo "  - Registry: $NMSTATE_REGISTRY"
echo

# Create CRD file
cat <<'EOF' > nmstate-crd.yaml
---
            ...
            properties:
              affinity:
                description: Affinity is an optional affinity selector that will be
                  added to handler DaemonSet manifest.
                properties:
                  nodeAffinity:
...
# Create namespace and RBAC files
cat <<'EOF' > nmstate-namespace.yaml
apiVersion: v1
kind: Namespace
metadata:
  name: nmstate
...
cat <<'EOF' > nmstate-deployment.yaml
apiVersion: apps/v1
kind: Deployment
metadata:
  name: nmstate-operator
    ...
    spec:
      ...
      containers:
        - name: nmstate-operator
          ...
          image: NMSTATE_REGISTRY/3rdparty/kubernetes-nmstate/kubernetes-nmstate-operator:v0.84.0
          ...
          env:
            ...
            - name: RELATED_IMAGE_HANDLER_IMAGE
              value: NMSTATE_REGISTRY/3rdparty/kubernetes-nmstate/kubernetes-nmstate-handler:v0.84.0
            ...
            - name: KUBE_RBAC_PROXY_IMAGE
              value: NMSTATE_REGISTRY/3rdparty/kubernetes-nmstate/origin-kube-rbac-proxy:4.10.0
EOF

# Replace variables in YAML files
echo "Replacing variables in YAML files..."
sed -i "s#NMSTATE_REGISTRY#$NMSTATE_REGISTRY#g" nmstate-deployment.yaml

# Apply the YAML files in order
echo "Applying NMState CRD..."
kubectl apply -f nmstate-crd.yaml

echo "Applying NMState namespace and RBAC..."
kubectl apply -f nmstate-namespace.yaml

echo "Applying NMState deployment..."
kubectl apply -f nmstate-deployment.yaml

echo "NMState installation completed!"

# Clean up YAML files
echo "Cleaning up temporary YAML files..."
rm -f nmstate-crd.yaml nmstate-namespace.yaml nmstate-deployment.yaml

...
# Triaging the SystemMemoryExceedsReservation Alert
## Issue
...
Walk the node's system slice if a single process dominates. `systemd-cgls` reads from `/sys/fs/cgroup`, which the debug pod sees through its host-namespace mount when started with `--profile=sysadmin`. `chroot /host` is rejected by ACP's cluster-level PSA — read host config files via the `/host` bind-mount instead.

```bash
# Pick an image that ships systemd-cgls (any image with systemd installed,
# image with the `systemd` package). Public registry images may not be
# pullable on isolated clusters; mirror or substitute.
kubectl debug node/$NODE -it \
  --image=<your-registry>/systemd-tools:latest \
  --profile=sysadmin \
  -- /bin/sh -c '
    systemd-cgls /system.slice/ | head -n 30
    cat /host/etc/node-sizing.env 2>/dev/null
  '
```

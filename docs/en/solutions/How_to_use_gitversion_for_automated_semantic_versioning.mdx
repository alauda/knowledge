...
- **`TrunkBased/v1`**: `Trunk-based` development with direct commits to `main`
...
<Tabs>
  <Tab label="Incremental Configuration">
...
data:
  GitVersion.yaml: |
    mode: ContinuousDelivery
    increment: Patch
    # Enhanced assembly informational format with git hash
    assembly-informational-format: '{Major}.{Minor}.{Patch}-{PreReleaseTag}.g{ShortSha}'
    branches:
      # Main branch: 3.18.1-beta.8
      main:
        mode: ContinuousDelivery
        regex: ^(main|master)$
        label: "beta"
        ...
        prevent-increment:
          when-current-commit-tagged: false
      # Release branch: 3.16.0-rc.7
      release:
        mode: ContinuousDelivery
        regex: ^releases?[/-]v?(?<version>\d+\.\d+(?:\.\d+)?)$
        label: "rc"
        increment: Patch
      # Development branch: 3.19.0-develop.7
      develop:
        mode: ContinuousDelivery
        regex: (?<BranchName>^dev(elop)?(ment)?$)
        label: "{BranchName}"
        increment: Minor
      # Pull request: 3.18.0-pr534.22
      pull-request:
        mode: ContinuousDelivery
        regex: ^(pull-requests|pull|pr)[\/-](?<Number>\d*)
        label: "pr{Number}"
        increment: Inherit
      # Conventional commits: build/*, chore/*, ci/*, docs/*, feat/*, fix/*, etc.
      conventional:
        mode: ContinuousDelivery
        regex: ^(?<phase>(build|chore|ci|docs|feat(ures?)?|fix|hotfix|perf|refactor|revert|style|test))[/-](?<BranchName>.*)
        label: "{phase}-{BranchName}"
        increment: Patch
      # Hotfix branches: hotfix/critical-fix -> 3.18.1-hotfix.1
      hotfix:
        mode: ManualDeployment
        regex: ^hotfix(es)?[\/-](?<BranchName>.+)
        label: "hotfix"
        increment: Patch
        is-release-branch: true
      # Support branches: support/v2.0 -> 2.0.1
      support:
        mode: ContinuousDelivery
        regex: ^support[\/-](?<BranchName>.+)
        increment: Patch
        is-main-branch: true
      # Unknown branches: abc/def -> 3.18.2-devdef.1
      unknown:
        mode: ContinuousDelivery
        regex: (.*[-/])?(?<BranchName>.+)
        label: "dev{BranchName}"
    ...
    - image: $(params.gitversion-image)
      ...
      script: |
        ...
        if [  -f GitVersion.yaml ]; then
          ...
          $(gitversion -showconfig) > $FINAL_GITVERSION_CONFIG || true
        fi
        ...
        echo -n "$(gitversion $REPO_PATH /nocache /config $FINAL_GITVERSION_CONFIG)" | tee $FINAL_RESULTS_PATH || true

...
For detailed information about these branching strategies and their implementation patterns, see: [GitVersion Branching Strategies](https://gitversion.net/docs/learn/branching-strategies/overview)
...
> You can save the following definition as `git-version.yaml`.
  ...
  steps:
    - image: $(params.gitversion-image)
      ...
      script: |
        ...
        ## Checking for config priority: custom config > repo config > default config
        if [ "$(workspaces.config.bound)" = "true" ] && [ -f "$CUSTOM_CONFIG" ]; then
          ...
          echo "==> No GitVersion.yaml found, generating default config..."
          gitversion /showconfig > $FINAL_GITVERSION_CONFIG
        fi

        if [ "$(workspaces.config.bound)" = "true" ] && [ ! -f "$CUSTOM_CONFIG" ]; then
          echo "WARN: config workspace is bound but no $(params.config-file) file was found... using fallback config..."
        fi
...
- Unexpected version format? → [Configuration Issues](#configuration-issues)
...
# Solution: Update GitVersion.yaml
branches:
  main:
    increment: Minor
    prevent-increment:
...
- [`GitVersion` Branching Strategies](https://gitversion.net/docs/learn/branching-strategies/overview) - `GitFlow`, `GitHub Flow`, and `trunk-based` development patterns

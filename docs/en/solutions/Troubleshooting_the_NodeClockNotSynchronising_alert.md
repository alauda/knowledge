...
1. Ensure the chrony service is running and has at least one reachable upstream source. From a shell on the node (use `kubectl debug node/<name> -- chroot /host` when you do not have SSH):

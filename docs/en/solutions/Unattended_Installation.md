...
Options:
    ...
    --cri-version                       CRI type and version. Default: `containerd://1.6.20-4`.
                                        * You can obtain the supported Kubernetes and CRI versions as follows:
                                          ...
                                          * Where the `<TOKEN>` can be retrieved with `curl ${INSTALLER_IP}:8080//cpaas-installer/api/token`.
    --self-vip                          Whether to enable a self-managed VIP. Default: `false`.
    --vrid                              VRID used by the self-managed VIP. Range: `1–255`. VRIDs must be unique within the same subnet. Default: `137`.

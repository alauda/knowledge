...
Confirm the daemon is the one running, that it is enabled in systemd, and that the active configuration carries the intended `server` lines on every node [ev:c3]:
...
The expected output is a single `chronyd` process invoked as `/usr/sbin/chronyd -F 1`, the unit reporting `enabled` and `active (running)`, and the file showing the operator-supplied `server` entries with the distribution `pool` lines commented out — identical on every node in the cluster [ev:c3].

...
A hit in a namespace other than the Pod's confirms the resource exists but is not reachable from the Pod by the standard reference fields — the resolution is to create a copy in the Pod's namespace rather than to attempt a cross-namespace reference.

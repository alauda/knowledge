...
# Moving etcd to a Dedicated Disk on Control Plane Nodes
## Issue
...
3. **Drain and migrate the etcd member.** On the target node:
   ...
   # configuration.
   kubectl debug node/$NODE -it --image=<image-with-shell> \
     -- sh -c '
...
for n in $(kubectl get node -l node-role.kubernetes.io/control-plane -o name); do
  kubectl debug "$n" -it \
    --image=<image-with-shell> \
    -- sh -c 'df -h /var/lib/etcd && mount | grep /var/lib/etcd'
done
```

...
The commands below pull the evidence in the table's own order — **the decisive fields print first** (the parent run's `spec.status` and `cancel-reason`, each child TaskRun's `spec.status` / `spec.statusMessage`), then Events and PolicyReports as corroboration:
...
collect_cancel_evidence() {
  ...
  # Decisive attribution fields FIRST (the table's own order): the cancellation-class

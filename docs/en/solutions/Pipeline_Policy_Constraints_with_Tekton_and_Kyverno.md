...
| 5 | DAG dominance | Release-type side-effect tasks must be ordered after the gate | T | — (template responsibility; hook and trade-offs for building your own Audit: end of the [§4.1](#s4-1) intro) |
| 6 | finally safety | No gate-protected side effects inside finally | T | — (template responsibility; finally execution semantics: [§4.2.2](#s4-2-2)) |
...
5. **DAG dominance** (T): **every** release / push / promotion side-effect task must depend on the gate transitively (`runAfter`, directly or indirectly). The gate can only stop its DAG successors — **tasks ordered before or parallel to the gate may already have finished, and failure does not roll back side effects that already happened**. Making side effects dominated by the gate is a template design responsibility; this document ships no ready-made DAG-dominance Audit (judging transitive dependency means computing a closure — trade-offs at the end of the [§4.1](#s4-1) intro), and the resolved-definition snapshot of [§4.1.4](#s4-1-4) (which contains `runAfter`) is the hook for building such a criterion yourself.
6. **finally safety** (T): finally tasks execute when the pipeline fails, or when it is cancelled with **`CancelledRunFinally`** (whether finally runs under deny vs. cancel is contrasted in the table in [§4.2.2](#s4-2-2); the full trade-off across the three response shapes is in [§4.2.3](#s4-2-3)); a plain `spec.status: Cancelled` does not guarantee scheduling of finally tasks that have not started — so finally must not contain any gate-protected side effect (release, push). There is likewise no ready-made Audit of finally contents (the [§4.1.4](#s4-1-4) snapshot contains the `finally` list; you can build one — same trade-offs as above).
...
| 7 | Release-type tasks not dominated by the gate, or a gate-protected side effect placed in finally | 🟡 | Contracts 5 / 6 are template design responsibility; the K side ships no ready-made criterion ([§4.1.4](#s4-1-4) audits only the gate's identity; its snapshot is the hook for building such an Audit). **The definition-side admission route was not taken in this document**; its shape and cost are at the end of the [§4.1](#s4-1) introduction |
...
| [§4.5.3](#s4-5-3) image allowlist | C | `containers[].image`; `initContainers` likewise, as an extra block; for the `ephemeralcontainers` cell use `kubectl patch pod <name> --subresource=ephemeralcontainers` instead (it is a subresource UPDATE, not a CREATE). **Skeleton C's `managed-by` label must be substituted with the real value per [§4.0.3](#s4-0-3)** — every rule in that section scopes to Tekton Pods by this label; with the label missing or wrong, every cell skips at match time, the "approved image" and "unapproved image" cells come out identical, and the probe as a whole is void |
| [§4.5.4](#s4-5-4) bare-entry closure | B | The manifest barely changes; **what varies is the identity**: run the same request once with `--as=<platform-admin-identity>` and once with `--as=<business-identity>` — only when the two cells come out opposite has this policy been hit. ⚠️ **That allow/deny pair only exercises the break-glass branch** — the allow side must also cover the [§4.5.4](#s4-5-4) verification-checklist cells "controller identity + controller owner ref allowed" and "normal end-to-end PipelineRun `Succeeded`": with the controller + ownerReference normal path broken in the policy (always false), the admin branch still allows and the business identity is still denied, so this recipe false-passes while real pipelines can no longer create child TaskRuns; for the CustomRun cell change `kind` to `CustomRun`, `taskRef` to `customRef`, **and `apiVersion` to `tekton.dev/v1beta1`** — CustomRun exists only in v1beta1, `tekton.dev/v1` does not have it (the exception in [§3.2](#s3-2) "API group-version premise"); copying Skeleton B's v1 gets rejected by the API server before admission is ever reached |
...
| [§5.2](#s5-2) namespace layering | A | Change `metadata.namespace` and `-n` **together** to the target namespace (`proj-a` / `proj-b` / `rogue-ns` — the two must agree: changing only `-n` while the manifest still says `policy-poc` makes kubectl fail **before admission is ever reached**, with `the namespace from the provided object "policy-poc" does not match the namespace "proj-a"` — the probe never reaches the policy), then add the field that violates the baseline / the project policy per that section's body |
| [§5.3](#s5-3) exemption boundary | A | Switch `metadata.namespace` and `-n` **in step** between `policy-exempt-runs` and an ordinary namespace (a mismatch is rejected before admission, as in the row above), set both gate switches in `spec.params` to `"false"`, and carry `--as` throughout |
...
|---|---|---|
| **External calls on the admission path (synchronous admission)** | Three criteria wait for an external round trip inside the admission request: `context.imageRegistry` ([§4.5.2](#s4-5-2)), the [§4.2.1](#s4-2-1) validate `context.apiCall`, and the [§4.2.3](#s4-2-3) admission-mutate `context.apiCall` (which fetches the parent PipelineRun — **although it hangs off a mutate rule, it executes synchronously inside the TaskRun CREATE webhook request and draws on this row's budget just the same; do not leave it out of the count**). **All three fail closed**: registry unreachable → request denied (roughly 5 seconds; roughly 3 seconds when reachable, see limitation 4 in [§4.5.2](#s4-5-2)); the [§4.2.1](#s4-2-1) / [§4.2.3](#s4-2-3) apiCall cannot fetch its target → the rule errors out and the request is denied (an admission-mutate rule failure is blocked per the policy's `failurePolicy` just like a validate failure — mechanism source: the `BlockRequest` branch of release-1.15 `pkg/webhooks/resource/mutation/mutation.go`; the [§4.2.1](#s4-2-1) error message shape is in that section's warning). **"Error means deny" presupposes that the policy's effective `failurePolicy` is `Fail`** (this document's synchronous interception assets all declare `Fail` explicitly; the full tiering is in "The `failurePolicy` trade-off" row below); with it switched to `Ignore`, or with the platform-level `forceFailurePolicyIgnore` in effect, the same error turns into an allow. **The apiCalls of [§4.2.2](#s4-2-2) / [§4.6.1](#s4-6-1) do not belong in this row** — they hang off mutate-existing and fail in the opposite direction; see the next row | Decide explicitly "which request paths may carry external calls"; narrow such rules' match down to **the Tasks that genuinely need them**; load-test p95 / p99 and the timeout ratio, and confirm it stays below the webhook timeout — **that ceiling applies to the whole request, not to each rule separately** (default `timeoutSeconds=10`, see checklist item 6 in [§3.1](#s3-1); one 5-second registry round trip fits, two stacked on the same request may not). **Registry / API server jitter turns directly into pipeline creation failures** — have the matching alerts and playbooks ready |
| **The asynchronous delivery chain (mutate-existing cancellation is fail-open)** | Of the document's four cancellation paths, three are mutate-existing ([§4.2.2](#s4-2-2) / [§4.6.1](#s4-6-1) / [§4.6.2](#s4-6-2)), and they sit **outside the admission verdict**: on a hit, the background-controller patches the target object asynchronously via an UpdateRequest. So when any link in that chain fails — the rule's `context.apiCall` cannot fetch its target, the UpdateRequest never gets created, the background-controller is down or backlogged, update RBAC on the target resource has been revoked — **the original request is allowed as usual and the cancellation patch silently vanishes**: the pipeline runs to the end, with no denial message anywhere in the cluster and no PolicyReport violation record (mutate types produce no violation records — the [§4.2.3](#s4-2-3) warning), at most a few error lines left in controller logs (evaluation and write layer in the background-controller, the UpdateRequest **creation layer in the admission-controller** — the three layers' signatures are in [§3.7.1](#s3-7-1) ③; **the failure class where the controller itself is unavailable leaves not even those lines** — only the liveness monitoring of [§3.7.1](#s3-7-1) ① covers it). **Criterion direction fail-closed ≠ delivery guarantee**: the criterion says "missing/illegal results must cancel just the same", but whether the cancel lands depends on the health of this background chain | **For a hard guarantee of zero races and zero silent failures, only the synchronous paths qualify**: the deny of [§4.2.1](#s4-2-1) or the admission mutate of [§4.2.3](#s4-2-3) (both give a synchronous verdict inside admission). If you stay on mutate-existing, you must monitor this chain as **a delivery system with an SLA**: the usable signal surface (controller liveness / metrics / logs and events), each signal's semantic boundary, the attribution method, and the controlled fault-injection procedure are in [§3.7.1](#s3-7-1) — the **single complete version** of this monitoring contract ([§3.8](#s3-8) step 9 and the [§6.2.3](#s6-2-3) troubleshooting only reference it, never restate it); write the chosen signals and the alert-triage SOP into the change request |
...
| **The `failurePolicy` trade-off** | **The tiering lands in each policy's own `spec.webhookConfiguration.failurePolicy`** (field semantics, the two-layer reading and the check commands are in checklist item 6 of [§3.1](#s3-1) and in [§3.1.2](#s3-1-2); the consequences of the two values are in [§6.1.8](#s6-1-8) — not restated here). ⚠️ **`validate.failureAction: Audit` does not shield you from this**: it only decides whether a successfully evaluated rule blocks; **the policy's match surface registers in the webhook regardless** — while Kyverno is unavailable, the API server disposes of every matching request per that policy's `failurePolicy`; an Audit policy declaring `Fail` rejects the matching `*/status` write-backs just the same, pipelines stay stuck for as long as Kyverno is down, and this is a policy that never blocks anything in normal operation. This document's assets are tiered and declared accordingly: **17 declare `Fail`, 8 declare `Ignore`**. The `Fail` tier = the admission-interception policies (Enforce / mutate-inject) **plus the cancellation trigger of [§4.2.2](#s4-2-2)** — mutate-existing though it is, its trigger surface is a main-resource TaskRun CREATE and it guards the very gate [§4.2.1](#s4-2-1) guards (one of the three response forms — see the comparison table in [§4.2.3](#s4-2-3)); with `Ignore`, a Kyverno outage would let violations through **neither blocked nor cancelled, permanently** (`background: false`, no retro-scan). The `Ignore` tier holds 8 — 7 of them match `*/status` (status write-backs must never be blocked by a Kyverno outage — the [§2.2](#s2-2) red line holds in the failure scenario too), and 1 is the inventory scan of [§4.4.4](#s4-4-4) (background Audit matching the PipelineRun main resource: it blocks nothing in normal operation, yet with `Fail` a Kyverno outage would reject every matching PipelineRun CREATE — zero interception benefit, pure availability cost; re-evaluate the tier when promoting it to Enforce per [§4.4.4](#s4-4-4)) | Tier by policy risk, written into the policy body and managed with GitOps: **the platform baseline and the Pod-level image allowlist stay on `Fail`**, provided the four controllers are HA across nodes and stay available through rolling upgrades; bookkeeping-only Audit policies and the `*/status`-matching cancellation triggers ([§4.6.1](#s4-6-1) / [§4.6.2](#s4-6-2)) take `Ignore` (8 assets in this document: the 7 `*/status` matchers + the [§4.4.4](#s4-4-4) inventory scan; **the [§4.2.2](#s4-2-2) cancellation trigger is not in this tier** — it guards the same admission gate as [§4.2.1](#s4-2-1) and stays on `Fail` with it, rationale in the left cell) — the cost is that this bookkeeping and cancellation triggering go missing while Kyverno is unavailable, so write down the accepted vacuum boundary. **Two things not to conflate**: ① on a mutate-existing cancellation policy this field only decides whether the **triggering request** is blocked while the Kyverno webhook is unavailable — `Fail` does not close the fail-open of the asynchronous delivery itself (the row above); ② once the platform-level `forceFailurePolicyIgnore` is on, every declared `Fail` is defeated — a platform-wide override switch, not a tiering tool (mechanism and check method in [§3.1.2](#s3-1-2)). For every cluster, write down "which tier was chosen + why + minimum replica count + what happens during a Kyverno maintenance window", and rehearse one failure scenario |
...
#### 3.7.1 The asynchronous delivery chain's monitoring contract (signal surface, attribution, and fault injection) {#s3-7-1}

This subsection expands the "monitor it as a delivery system" requirement of the "asynchronous delivery chain" row above, and is the **single source of truth** shared by [§3.8](#s3-8) step 9 (upgrade regression) and [§6.2.3](#s6-2-3) (per-run troubleshooting) — those two only reference this subsection and never restate it; signal-semantics revisions land only here.

First, rule out the action most often mistaken for evidence: **an after-the-fact one-shot `kubectl get updaterequests` is not a data source** — a failed UpdateRequest is visible only during its retry window and is deleted once the retries end (typically within a minute or two), after which the query is empty, and **empty output proves nothing about chain health**. The usable data sources are ①–③ below (⚠️ ③ spans **two controllers, admission and background** — the chain's first link, "create the UpdateRequest", happens on the admission side, and watching only the background side misses that whole layer), with ④ as their controlled verification; **none of these signals can by itself prove "the patch landed in the target" — the final arbiter of chain health is always the target object's end state** (the primary criterion of [§3.8](#s3-8) step 9):

- ① **Background-controller liveness, restart count, and queue backlog.** This one is the precondition for the rest: **with the controller down, neither ② nor ③ produces any new record** — "the controller is dead" looks exactly like "all quiet" in ② and ③.
- ② **Metrics (persistent counters — ⚠️ they cover the rule-evaluation layer only)**: on the background-controller's metrics port (`kyverno-background-controller-metrics:8000`), `kyverno_policy_results_total{rule_type="mutate",rule_execution_cause="background_scan"}`, labelled by `policy_name` / `rule_name`, persistent across UpdateRequest deletion (being a counter it resets on controller restart — feed it into a scraping system before alerting on it).
  - **This counter records the rule-evaluation outcome and is bumped before the patch is ever written to the target**: `rule_result="pass"` = evaluation succeeded, a patch was computed — **it does not prove the patch was written into the target object**; `rule_result="error"` = evaluation failed (including an apiCall that cannot fetch its target). If the subsequent target write then fails on RBAC, a resourceVersion conflict, or an API error, **`pass` has already been counted and `error` never increments — write-layer failures are entirely invisible in this counter**; only ③'s second log signature and the events cover them.
  - It also **counts attempts, not incidents** (the initial delivery and every retry each add one line), and `error` has a benign source: when the parent object is already deleted, the apiCall 404 is a normal cleanup race (the 404 note of [§4.6.1](#s4-6-1)) and is counted as `error` all the same. So this counter **is an evaluation-layer triage clue, never a direct alert condition** — "any increment > 0 alerts" gets steadily false-positived by normal cleanup races — and **never a "hits minus landings" reconciliation**.
  - When `error` increments, **attribute by target object**: find a run that "should have been cancelled but is still running" (check `spec.status` / `cancel-reason` — the per-run commands of [§6.2.3](#s6-2-3)); only such a run is a genuine delivery failure. **Attribution has a shelf life**: once the target object is GC'd or cleaned up, "the object was already deleted when the error occurred (benign 404)" and "delivery genuinely failed and the object vanished afterwards" leave identical scenes ([§6.2.3](#s6-2-3) also notes that annotations / events / reports all vanish with the object) and can no longer be told apart — so put the follow-up check into the alert SOP and **run it promptly** (within the object-retention window), and record unattributable increments as "unknown, needs investigation" — **never default them to benign**.
- ③ **Logs and events (deterministic signatures — creation / evaluation / write, each layer with its own home)**:
  - **Creation-layer** failures (the UpdateRequest never gets created) log **in the admission-controller, not the background-controller**: creation runs in an asynchronous goroutine started at admission time (`Apply()` returns immediately; the error never reaches the admission verdict), and after ~3 seconds of backoff retries the failure logs ERR `failed to update request CR`; when the cluster's UpdateRequest count reaches `updateRequestThreshold` (the Kyverno ConfigMap), creation is **skipped outright** with ERR `UpdateRequest creation skipped`. When this layer fails, ① and ② and the background-side logs are **all silent**, and a pre-started watch shows nothing either (no object was ever created) — mechanism source: release-1.15 `pkg/webhooks/updaterequest/generator.go` (`Apply()` / `applyResource()`) and the threshold branch of `pkg/utils/generator/updaterequests.go`.
  - **Evaluation-layer** failures log ERR `failed to mutate existing resource, rule <rule-name>, ...` (when the apiCall cannot fetch its target the error string contains `failed to fetch data for APICall`), with structured `policy=` / `resource=` fields. This line is **not fit for bare alerting** (the normal already-deleted-parent race logs the very same line); look up the target object by `resource=` and classify per ②'s attribution criteria. On this path the target object was never resolved, so **no Kubernetes event is produced** (Kyverno's log says so verbatim: `cannot generate events for empty target resource`).
  - **Write-layer** failures (evaluation passed, writing the patch to the target failed) log **a different** ERR signature, `failed to update target resource` (with the target's `namespace=` / `name=` fields) — search **all the signatures, across both controllers**; searching only the first one drops the entire write-layer failure class. Here the target object has been resolved, so Kyverno emits a `BackgroundFailed` (success side: `BackgroundSuccess`) event on the target object, visible via `kubectl get events` in the target namespace — the write layer's only object-level signal (mechanism source: the Pass branch and `report()` of release-1.15 `pkg/background/mutate/mutate.go`).
- ④ **Controlled fault injection (once before rollout and after every upgrade)**: **start `kubectl get updaterequests -n kyverno -w` before triggering** (the UpdateRequest lifecycle is visible only to a pre-started watch — the failure side shows `Pending → Failed` retries and then disappears, the healthy side shows `Pending → Completed`; **a confirmed trigger with nothing ever appearing in the watch = a creation-layer failure** — go to the admission-controller log for ③'s two creation-layer signatures); on shared / production clusters use **a dedicated test policy + a test namespace + a harmless apiCall pointing at an object known not to exist** (touching no real policy and no business object), confirm the signals of ② and ③ actually appear and that the attribution step pinpoints the injected target object, and write the result into the change request. "Temporarily stopping the background-controller" is a cluster-level outage (it simultaneously interrupts every other mutate-existing / generate delivery on that cluster) — reserve it for a dedicated acceptance cluster or a maintenance window, record the replica count before, and verify Ready plus backlog drain afterwards.
...
Run in order. **The requirement "one probe that should be allowed + one that should be denied" holds only for the admission-Enforce steps** — steps **2 / 3 / 7 / 8 / 10** must run both, and running only half cannot catch a direction error (the reasoning is the two-step self-check in [§4.0.3](#s4-0-3)). The other five kinds have no **"admission-allowed"** side, and each has its own pass criterion — do not force the pattern onto them: step **1** is a configuration diff (it submits no objects); steps **5 / 6** are Audit — Audit blocks no requests, so every request gets "allowed", but **that does not mean they have no healthy side**: the healthy side lives in the PolicyReport (normal input must come out `pass` / `skip`, not `fail`), so these two steps also run both sides — with only the violating side, a broken policy that records **every** input as `fail` (the typical shape of an identity criterion wired backwards) passes the regression untouched; step **4** verifies that "the response shape matches the one you chose" (a three-way pick, and deny vs. the cancellations are checked in different places), while step **9** verifies whether the cancellation **actually reached the object** (mutate-existing is asynchronous; the admission side does not deny); step **11** is the end-to-end compliance baseline (allow side only — what it proves is that nothing compliant got hurt).
...
| 4 | One real gate TaskRun | Non-compliant parameters terminate in **the one** response shape you chose, and the three shapes are checked in different places ([§6.2.3](#s6-2-3)): [§4.2.1](#s4-2-1) deny → parent run `CreateRunFailed`; [§4.2.2](#s4-2-2) cancel the parent run → **the primary criterion is the parent run's `spec.status=CancelledRunFinally` plus the `cancel-reason` annotation** — the terminal state is usually `Cancelled`, but when the cancellation races a task failure (the result cannot be written, or the cancel lands inside a child TaskRun's initialization window) the terminal state is `Failed` while `spec.status` and the annotation are both present all the same (the [§2.3](#s2-3) master table; the two race notes in [§4.6.1](#s4-6-1)) — **that is not a regression failure; do not grade a healthy cancellation as an outage**; [§4.2.3](#s4-2-3) cancel the gate TaskRun itself → **that TaskRun** `Cancelled` + `spec.statusMessage` (the parent run shows no `cancel-reason`; do not judge failure by it) |
| 5 | One scan that **genuinely fails** (one for sonar and one for trivy) + **one clean scan as the healthy-side control** | Failing side: the corresponding `fail` appears in the PolicyReport. **No `fail` is always graded "acceptance not passed" — never "the scan passed", and never directly "the policy did not match"** — an unmatched policy identity, a `resourceFilters` skip, a report not yet converged, and an object already GC'd all produce the same empty result; work through the five meanings in [§4.4.4](#s4-4-4). Healthy side: the clean scan is recorded `pass` under this policy (the normal shape of [§4.4.1](#s4-4-1)), **with no unexpected `fail`** — without this side, a broken policy that records every terminal state as `fail` (an identity criterion wired backwards after the upgrade) is invisible to this step |
| 6 | Have the gate skipped via `when` / an empty matrix + **one run whose gate executes normally as the healthy-side control** | Violating side: it shows up in `status.skippedTasks`, and the [§4.1.5](#s4-1-5) Audit records the violation. Healthy side: the run whose gate executed produces **no** `fail` under this policy (the normal shape of [§4.1.5](#s4-1-5) is `skip` — the precondition does not hold) — otherwise a broken policy that judges **every** governed run as "gate skipped" passes this step untouched |
...
| 9 | When the cancellation policies of [§4.6](#s4-6) are installed: one run whose results miss the bar ([§4.6.1](#s4-6-1)) + one run with definition drift ([§4.6.2](#s4-6-2)) + **one compliant run of the same governed profile as a healthy-side control** (the `coverage-lines=85` reverse control from the [§4.6.1](#s4-6-1) checklist — step 11's compliant baseline is the `gated-build` fixture, not this step's governed profile, and cannot substitute) | **The primary criterion is the target run's end state**. Healthy side: the compliant run is neither cancelled nor carries a `cancel-reason` — without this side, a policy whose criterion direction flipped into "cancel unconditionally" passes this step regardless. Violating side: `spec.status` gets written to `CancelledRunFinally` with the correct `cancel-reason` wording (a metrics `rule_result="pass"` only proves the evaluation layer computed a patch, never that the patch landed in the target — the ② semantics of [§3.7.1](#s3-7-1)). **At the same time, confirm the asynchronous chain itself is intact — in a falsifiable way** (signal semantics per [§3.7.1](#s3-7-1); this step only adds the regression-specific judgment): either **start `kubectl get updaterequests -n kyverno -w` before triggering** and see this run's UpdateRequest reach `Completed` in the watch (`Pending → Failed` repeatedly and then gone = the chain is broken; the empty output of an after-the-fact one-shot get proves nothing — the opening of [§3.7.1](#s3-7-1)); or check the signal surface per [§3.7.1](#s3-7-1) ② and ③: attribute every `"error"` increment and every evaluation-layer ERR line during the window by its `resource=` — the targets inside a regression window are all objects **you** created, so the "attribution shelf life" of [§3.7.1](#s3-7-1) ② is naturally satisfiable here: match the ERR line's timestamp against the object's deletion time (you performed the deletion, or `kubectl get events` records it) — **only a matching timeline counts as a benign race; any signal without an establishable timeline is graded a failure**; and confirm the **absence** of the write layer's two signals (a `failed to update target resource` ERR line, and a `BackgroundFailed` event for this policy in the target namespace — [§3.7.1](#s3-7-1) ③). Any unattributable signal, or an acceptance run that never got cancelled, is a failure. This chain is precisely what an upgrade breaks most quietly (RBAC aggregation rule changes, UpdateRequest API version changes), and when it breaks there is no denial message of any kind |
...
| 11 | One complete business pipeline run to its terminal state (if none is handy, use `demo-run-pass` from [§3.3](#s3-3), the compliant fixture used throughout this document) | Check item by item rather than "it ran, good enough": the parent run's terminal state matches pre-upgrade; every child TaskRun in `status.childReferences` reaches its expected terminal state; finally executed; every Audit record that should be in the PolicyReport is there, **and there is no unexpected `fail`** (pull them by run UID with the [§6.2.3](#s6-2-3) commands — a `fail` popping up under a compliant run means some Audit policy's criterion got wired backwards in the upgrade; the healthy-side controls of steps 5 / 6 exist to catch exactly that); and not a single `cancel-reason` or `statusMessage` appears unexpectedly |
...
- The **cancellation-type** policies of [§4.2.2](#s4-2-2) / [§4.2.3](#s4-2-3) / [§4.6](#s4-6) — response actions, not admission blocking; of them, [§4.2.2](#s4-2-2) and [§4.6](#s4-6) are mutate-existing and need the extra RBAC ([§4.6](#s4-6) introduction), while [§4.2.3](#s4-2-3) is an admission mutate and needs **no** extra RBAC;
...
One policy name per table row. `pod-image-registry-allowlist` ships two interchangeable YAMLs, so the row **immediately below it** describes the same-named alternative shape — not another policy.

**First, one thing that holds for every policy**: each policy in this chapter is scoped to the demo namespace `policy-poc` (the [§4](#s4) introduction already flags this as a demo choice; two gate policies additionally carry the demo exemption namespace `policy-exempt-runs` in their `namespaces` lists — see the warning in [§4.0.7](#s4-0-7) step 1). So **whether a row says ✅ or 🔧, you must change the scope before copying it to production** — to the namespaces you actually govern, or into a namespaced `Policy` per [§5](#s5). The column below answers a different question: **beyond scope and placeholders, do the identity criteria inside the policy also need changing?**
...
| `<tekton-default-service-account>` | The SA name Tekton defaulting fills into a run-level `spec.taskRunTemplate.serviceAccountName` (the first entry of the approved list in [§4.5.5](#s4-5-5) — **this entry is not optional**: the defaulting webhook runs before Kyverno, and leaving it off the list amounts to refusing to approve "ordinary requests") | Do not guess from `config-defaults` (a missing key and an explicitly empty value mean different things — see the "effective value comes back empty" paragraph in [§4.5.5](#s4-5-5)); read the **effective value** after defaulting instead (using the [§3.3](#s3-3) fixture run; `--dry-run=server` has no side effects; factory state prints `default`): `kubectl create --dry-run=server -n policy-poc -f demo-run-pass.yaml -o jsonpath='{.spec.taskRunTemplate.serviceAccountName}{"\n"}'` — it reads exactly the field the policy compares; whatever it prints is what you fill in. ⚠️ **Empty output must be dealt with**: it means the key exists with an empty value and Tekton skipped the fill-in — the `runWideSa != ''` criterion of [§4.5.5](#s4-5-5) then skips wholesale (a fail-open); mechanism and fix in that section's "effective value comes back empty" paragraph | ⚠️ **Miss it = 100% of compliant deployment-enabled requests rejected**. The self-check is that allow probe: one real release run must be allowed. **Also run the effective-value command once and confirm the output is non-empty** — empty output = the fail-open in the previous column |
...
- **Cancellation (mutate-existing / admission mutate) has no rejection message**: the user only sees `Cancelled`. **The reason must be written into the object by you** (the `cancel-reason` annotation or `spec.statusMessage`) — otherwise the object retains not even a clue that "a policy did this" ([§6.2.3](#s6-2-3)).
...
1. **Swap the scope**: replace the `policy-poc` listed under `namespaces` in every policy with the range you actually govern. Three things all happen in this step:
   - **Pick one of the four scope shapes** ([§5.1](#s5-1)): **enumerating** namespaces one by one, a `namespaceSelector` selecting by Namespace labels, "platform-level `ClusterPolicy` + **negative `exclude`** carving out system namespaces", or simply **converting to a namespaced `Policy`** for project self-service (the second layer of [§5.2](#s5-2)). For "covered by default, newly created namespaces picked up automatically" you must use the negative-`exclude` shape — enumeration and label selection both naturally miss namespaces created later (the first row of [§3.6](#s3-6)).
   - ⚠️ **Two policies list more than `policy-poc` under `namespaces`**: `gate-param-contract` ([§4.2.1](#s4-2-1)) and `gate-param-mutate-to-cancel` ([§4.2.3](#s4-2-3)) also carry the demo exemption namespace `policy-exempt-runs` (serving the [§5.3](#s5-3) exemption demo) — replacing only `policy-poc` carries that demo exemption entry straight into production; delete it during conversion, or replace it with the production exemption namespace you establish through the [§5.3](#s5-3) governance process.
   - ⚠️ **The demo namespace is not pinned only in `match` — three more places must change with it** (`match` / `targets` / RBAC are three independent, non-linked settings — [§4.2.2](#s4-2-2) warning ① explains this; here is where they land in the conversion): ① the `mutate.targets[].namespace: policy-poc` **literals** in [§4.2.2](#s4-2-2) and [§4.6.2](#s4-6-2) ([§4.6.1](#s4-6-1) uses the `{{ request.namespace }}` variable and needs no change) — replace with your target-namespace literal, or switch to the variable + aggregated ClusterRole per the [§4.6](#s4-6) introduction; ② the `namespace` of the companion namespaced `Role` / `RoleBinding` (the RBAC details block of [§4.2.2](#s4-2-2)) changes with it; ③ in the ConfigMap shape of [§4.5.3](#s4-5-3), the `context.configMap.namespace: policy-poc`, the `pipeline-image-allowlist` ConfigMap object itself, and the `-n` of its check commands — miss this one and, the moment the demo namespace is cleaned up, the policy fail-closes across the whole production scope (the direction of a missing ConfigMap is in [§4.5.3](#s4-5-3)). Change only `match` and the cancellation policies will hit production requests yet look for their target in the demo namespace (or lack update permission on the target namespace) — the expected cancellation silently never happens. run the **two-layer check** of [§3.1](#s3-1) item 6 against the target cluster — the declaration alone is not enough: a declared `Fail` whose webhook lands in the `-ignore` group is being force-overridden by the platform-level switch; resolve the override before discussing tiers (mechanism in [§3.1.2](#s3-1-2)). Confirm each policy's tier matches this cluster's availability plan (the tiering of [§3.7](#s3-7); this document's assets ship tiered as "interception plus the [§4.2.2](#s4-2-2) gate cancellation trigger `Fail`; bookkeeping and the `*/status` cancellation triggers — including the [§4.4.4](#s4-4-4) background inventory — `Ignore`"); a policy whose tier this cluster's plan wants different gets changed here, with the accepted vacuum boundary written into the change request.
...
| Audit types | After the violating input, a `fail` entry for this policy **appears** in the PolicyReport; the compliant input gets `pass` / `skip` and **must not** produce a `fail` for this policy (a policy that records every input as `fail` passes the violating side alone — same healthy-side controls as steps 5 / 6 of [§3.8](#s3-8)) | Identity criteria pinned wrong → the rule skips → the report is clean, indistinguishable from "no violations" |
| Cancellation types (mutate-existing / admission mutate) | The target object genuinely shows a **cancellation-class** `spec.status` (`CancelledRunFinally` for the parent run, `TaskRunCancelled` for the gate TaskRun) and the matching policy marker — **the `cancel-reason` annotation on the parent run; `spec.statusMessage` on the gate TaskRun ([§4.2.3](#s4-2-3))** ([§6.2.3](#s6-2-3)) — judged on the **value**, not on "non-empty" | The request being allowed through as usual is **normal** (cancellation types do not reject requests), so "the submission succeeded" says nothing about whether the policy took effect |
...
So this document explicitly leaves contracts 3 / 5 / 6 on the template-design-responsibility side: contract 3 has a ready-made post-hoc Audit in [§4.1.5](#s4-1-5) reading `skippedTasks` (the one row of the [§2.3](#s2-3) table whose Guarantor reads `T + K post-hoc Audit`), while contracts 5 / 6 have **no ready-made Audit** — if you want to build one, the resolved-definition snapshot of [§4.1.4](#s4-1-4) (containing `runAfter` and `finally`) is the runtime-side hook, and the definition-side route below is the other place to land it. **If your templates are all in-cluster definitions and manageably few**, adding an Enforce policy on the definition side for "the gate must not carry a `when`; no release-class task may appear in `finally`" is worth it: those two criteria only read `spec.tasks[].when` and `spec.finally[]`, no transitive closure involved, and the cost lands on the third bullet above (configuring the gate name and the release-class task names per template). What it blocks is **definitions entering the cluster** — it is **complementary to, not a replacement for** [§4.1.5](#s4-1-5) reading the runtime `status.skippedTasks`: remotely referenced templates can still only rely on the latter. Roll out per [§3.5](#s3-5): Audit first, then Enforce.
...
**Expected shape**: in the violating run (explicit `false` or an explicit empty string), `prep` succeeds, `scan` is cancelled, `release` is skipped with `PipelineRun was stopping`, the parent run carries `spec.status=CancelledRunFinally` with terminal state `Cancelled` (which can be `Failed` when the cancellation races a task failure — whether the policy took effect is judged by `spec.status` and `cancel-reason`, [§2.3](#s2-3) / [§4.6.1](#s4-6-1)), the `cancel-reason` annotation is present, and **the finally `notify` succeeds normally**; in the compliant run, `prep / scan / release / notify` all succeed and the parent run is `Succeeded`.
...
Paired with the "Audit first" rollout process of [§3.5](#s3-5), its value is turning governance into a **measurable process**: first inventory the baseline with Audit + `background: true` (reading 3 gives you a number), hand the list to the teams for remediation (reading 2), and switch to Enforce once the number drops to 0 or an acceptable level — and **re-evaluate the `failurePolicy` tier in the same change**: this policy ships as `Ignore` (pure bookkeeping has no interception benefit; see the asset comment and the [§3.7](#s3-7) tiering), but switched to Enforce it joins the interception class — decide per the cluster's availability plan whether it goes back to `Fail`. After remediation the reports converge on their own — patch the label onto a violating run and its report flips from `fail: 1` to `pass: 1` at the next evaluation; no manual report cleanup is needed.
...
**When `<tekton-default-service-account>`'s effective value comes back empty, fix the ConfigMap before discussing the policy** (this paragraph is the complete version of that placeholder's fail-open mechanism; the value row in [§4.0.3](#s4-0-3) points here): always take the value with the [§4.0.3](#s4-0-3) effective-value command, which reads the result **after** defaulting — do not guess from `config-defaults`. A **missing** `default-service-account` key falls back to Tekton's built-in default `default` (most ACP clusters ship with the key missing: it appears only in the inert `_example` comment block, so reading an empty value straight off the ConfigMap is normal, not a broken command). A **key that exists with an empty value** is a different case: when Tekton reads this config it only checks whether the key **exists** — if present, the value is taken as-is, so an empty string overrides the built-in default `default`; and the fill-in happens only when the default value is non-empty (for PipelineRun and TaskRun alike) — so admission **really does see the field absent**, and this section's `runWideSa != ''` criterion skips wholesale: **a fail-open**. The right fix is to change the empty value in `config-defaults` to a real SA name (same reasoning as `<tekton-managed-by-label-value>`: an empty value is itself a deployment blocker); left unchanged, the rule is as good as absent.
...
**Shared prerequisite of the last three rows (the three mutate-existing paths)** ([§4.2.3](#s4-2-3) is an admission mutate touching the incoming request object and does not need this permission; for the other three, missing it the policies either fail to install or never take effect): the background controller needs update RBAC on the target `pipelineruns`, and **Kyverno validates that RBAC at policy-creation time — if it is missing, creation of the policy is rejected outright**, with an error like this:
...
**① When the parent run has already been deleted**, the `apiCall` errors with a 404 — fail-safe for the cancellation scenario (with the parent gone, there is nothing to cancel anyway), but it leaves error lines in the background-controller log **and bumps the metrics `rule_result="error"` just the same**; no need to panic during troubleshooting. And precisely because this normal race shares its entire signal set with a genuine delivery failure, the monitoring items of [§3.7](#s3-7) require attributing by target object before classifying — never alerting on a bare `error` increment ([§6.1](#s6-1)).
...
⚠️ **This walkthrough asset carries only the cleanup-ownership token, not the governance metadata [§3.6](#s3-6) requires**: for production issuance, write the approval ticket, validity start / expiry, and owner into `metadata.annotations` as well — a machine-readable expiry is also what makes reclamation options ② / ③ above workable, and the issuance commands will not fill in these three for you.
...
⚠️ **Watch for the reverse as well: it should have been cancelled, but was not**. Shapes 2 / 3 / 4 are all mutate-existing — the cancellation is delivered asynchronously in the background; when a result is plainly out of bounds yet the pipeline runs to completion with neither `spec.status` nor `cancel-reason` on the parent run, the cause is usually not a missed criterion but a broken delivery chain (`context.apiCall` cannot reach the target, the UpdateRequest never got created, the background-controller is down or backlogged, the update RBAC on the target was revoked). This failure **produces no denial message and no PolicyReport violation record**. The full signal semantics (metrics cover the evaluation layer only, the three ERR signatures, the write-layer events, the attribution criteria and their shelf life) are governed by [§3.7.1](#s3-7-1); this paragraph only gives the troubleshooting **order**: **first confirm the background-controller itself is alive** ([§3.7.1](#s3-7-1) ① — with the controller down, none of the other signals produces any new record, and skipping this step makes "the controller is dead" look like "all quiet"); then search the logs — **all three layers' signatures, across both controllers** (background side: the evaluation-layer `failed to mutate existing resource` and the write-layer `failed to update target resource`; admission side: the creation-layer `failed to update request CR` / `UpdateRequest creation skipped` — when the UpdateRequest was never created, the background side is entirely silent; each layer's fields and events are in [§3.7.1](#s3-7-1) ③), attributing each line by `resource=` / target object (criteria and shelf life per [§3.7.1](#s3-7-1) ② — evaluation-layer signals mix in the normal already-deleted-parent race, the 404 note of [§4.6.1](#s4-6-1)); only a run that "should have been cancelled but is still running" is a genuine delivery failure — do not grade race noise as a chain outage; **do not treat an after-the-fact `kubectl get updaterequests -n kyverno` as evidence** (empty output proves nothing about chain health — the opening of [§3.7.1](#s3-7-1); to see UpdateRequests at all, start a `-w` watch before reproducing); check the criterion itself last — not because it cannot be wrong, but because a delivery failure leaves no denial or report trace and only the runtime signals above can falsify it, while the criterion can be re-checked offline (the probe skeletons of [§3.4](#s3-4)).

The commands below pull the evidence in the table's own order — **the attribution marker fields print first** (the parent run's `spec.status` and `cancel-reason`, each child TaskRun's `spec.status` / `spec.statusMessage`), then Events and PolicyReports as corroboration:

...
# Evaluating LoadBalancer Services with allocateLoadBalancerNodePorts=false and externalTrafficPolicy=Local
## Overview
...
# Pick any node and use any image that ships `ss` (iproute2). On
# isolated ACP clusters, public images like nicolaka/netshoot may not
# be pullable — substitute with whatever image your in-cluster
# registry mirrors.
NODE=<node>
kubectl debug node/"$NODE" -it --image=<image-with-iproute2> --profile=sysadmin -- \

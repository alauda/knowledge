...
Reproduce the doubled lookup deterministically with `dig` to confirm the pattern is search-path expansion and not application logic:

```bash
kubectl -n <ns> debug -it <pod> --image=registry.k8s.io/e2e-test-images/jessie-dnsutils:1.7 -- bash
# inside the debug pod:
dig +search +trace argocd-repo-server.argocd.svc.cluster.local
```

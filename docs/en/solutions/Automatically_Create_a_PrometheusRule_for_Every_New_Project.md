...
# Automatically Create a PrometheusRule for Every New Project
## Issue
...
metadata:
  ...
  labels:
    # The cluster Prometheus selector decides which rules are evaluated;
    # on ACP's stock kube-prometheus stack the selector is
    # 'prometheus: kube-prometheus'. Verify with the kubectl command below
    # and adjust this label to match whatever the cluster runs.
...
- **Label / selector compatibility.** The cluster's Prometheus instance has a `ruleSelector` (and a `ruleNamespaceSelector`) that decides which `PrometheusRule` objects it picks up. The labels on the templated rule must match that selector or the rule will be created but never evaluated. On ACP the platform Prometheus runs in `cpaas-system` (named `kube-prometheus`), and its `ruleSelector` requires `prometheus: kube-prometheus`. Confirm both with:

  ```bash
  kubectl -n cpaas-system get prometheus -o yaml \
    | yq '.items[].spec.ruleSelector,.items[].spec.ruleNamespaceSelector'
  ```

  and adjust the templated rule's labels to match — e.g. set `metadata.labels.prometheus: kube-prometheus` on the example rule below if the cluster ships the stock selector.
...
If the `PrometheusRule` appears but the alert never evaluates, the rule was not picked up by the cluster Prometheus. Check:
...
Verify the alert can fire by exercising the underlying expression. On ACP the Prometheus instance is the StatefulSet pod `prometheus-kube-prometheus-0-0` in `cpaas-system`, but its `prometheus` container is distroless and has no `wget` / `curl`. Query through the API server proxy instead:

```bash
kubectl get --raw \
  '/api/v1/namespaces/cpaas-system/services/kube-prometheus:9090/proxy/api/v1/query?query=ALERTS%7Balertname%3D%22VersionAlert%22%7D'
```

...
Pipelines-as-Code (PAC), maintained in the `Red Hat OpenShift Pipelines` project and tightly integrated with Tekton Pipelines, lets teams keep pipeline definitions versioned alongside application source so automation and code stay synchronized without drift.
...
> **Note**: Alternatively, the PAC CLI tool can be used to add repositories. See: https://pipelinesascode.com/docs/guide/cli/#repository-creation

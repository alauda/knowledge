...
Inspect the cgroup layout on a representative node to confirm v2 is actually in effect there:
...
Output showing `cgroup2 on /sys/fs/cgroup` confirms the node is v2. A `tmpfs on /sys/fs/cgroup` with nested v1 subsystems (`/sys/fs/cgroup/memory/`, `/sys/fs/cgroup/cpu/`) indicates v1 is still in effect on that node.

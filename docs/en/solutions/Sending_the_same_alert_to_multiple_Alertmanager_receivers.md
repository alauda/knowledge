...
On ACP the alerting stack is part of the observability component — specifically the Prometheus Operator based monitoring platform in `observability/monitor`. The Alertmanager configuration is stored in the `alertmanager-main` secret inside the observability namespace, and the routing tree in that secret is what needs the `continue` flag set.
...
Apply the updated configuration by re-creating the `alertmanager-main` secret in place. Save the edited YAML as `alertmanager.yaml` locally, then:

```bash
kubectl -n cpaas-system create secret generic alertmanager-main \
...
```bash
kubectl -n cpaas-system get secret alertmanager-main \
...
```bash
kubectl -n cpaas-system logs alertmanager-main-0
```

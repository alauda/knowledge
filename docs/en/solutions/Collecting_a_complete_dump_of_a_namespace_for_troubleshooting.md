...
Collect container logs for the whole namespace by listing its pods, reading each pod's container names from its spec, and pulling logs per container with timestamps. The container names come from `.spec.containers[*].name`, and `logs --container=<c> --timestamps` emits RFC3339-prefixed lines for each one.
...
For a container that has restarted, the logs of the prior terminated instance are retrieved separately with the `-p/--previous` flag, which returns the previous run's log rather than the current one.

...
ProductsVersion:
  - '4.1.0,4.2.x,4.3.x'
id: KB260500018
sourceSHA: 141c9cbd53294b8947aa426002eb8901232a6a89cad9a398278f4d1b4fe78ccd
---
...
在严重情况下，etcd 存活性探测开始失败，一个或多个 etcd 成员重启，这可能导致 API 服务器不可用，并在集群中引发级联警报。

## 根本原因

`rafthttp` 在每个与 etcd 对等体交换的心跳中嵌入一个时间戳。在每个数据帧中，接收方将嵌入的时间戳与其本地时钟进行比较；如果绝对差异超过一秒，则会发出上述警告。日志示例中的几分钟差异意味着两个或多个控制平面节点的系统时钟已经严重漂移。

由于 etcd 将心跳视为对等体存活的权威证据，严重的时钟偏差会与成员健康检查产生不良交互：一个时钟快几分钟的对等体可能会发送“未来”的心跳，这些心跳会被丢弃，而一个时钟滞后的对等体则可能完全错过心跳窗口。根本原因几乎总是相同的——至少有一个控制平面节点上的 NTP 未正常工作。

## 解决方案

将控制平面节点重新同步到 NTP。一旦时钟趋于一致，`rafthttp` 警告将在几个心跳间隔内停止，etcd 会自行恢复；不需要重启 etcd。

1. 确定哪些节点相对于其他节点存在时钟偏差。对每个控制平面节点运行 timedatectl 检查。ACP 的集群 PSA 拒绝 `chroot /host`；使用 `--profile=sysadmin` 和一个包含 `timedatectl`/`chronyc` 的镜像：
   ...
   注意 `System clock synchronized:` 行。如果为 `no`，则表示内核未确认 chrony（或节点操作系统中使用的其他 NTP 实现）保持时钟对齐。
...
3. 为了持久配置，通过节点配置通道更新节点 NTP 配置，以便替换节点继承相同的 NTP 设置。单个手动编辑的 `chrony.conf` 在活动主机上不会在节点替换时生效。
...
首先检查 etcd pod 日志，以确认哪些对等体受到影响——警告中的对等体 ID 告诉您是单个成员不同步还是整个集群漂移：
...
指向控制平面节点本身的 NTP 服务器列表，或指向一个已经消失的单一上游，是现场最常见的根本原因。将每个集群节点指向相同的一组外部（或负载均衡的）NTP 服务器，以便所有三个控制平面节点在同一参考时间上趋于一致。

...
ProductsVersion:
  - '4.2.x,4.3.x'
id: KB260100011
sourceSHA: caa4f197af8de2ff310a74ed515708c09591a9630cc089d291998367432c128f
---
...
- **VMware 环境**：
  ...
  - 网络连接：VMware 侧的 TCP 端口 **443** 和 **902** 必须开放，以允许来自 KubeVirt（ACP 集群节点）的访问。端口 443 用于 vCenter/ESXi API（SDK，NBDSSL 磁盘传输），端口 902 用于 NBD 磁盘传输到 ESXi。
- **机制说明**：Forklift 使用 ESXi 主机名构建迁移 Pod，以构造 `V2V_libvirtURL`，并通过 `esx://` 通过 SSH 连接以检索磁盘映像。

## 术语

在继续之前，请理解迁移过程中使用的以下关键概念：
...
1. 导航到 Alauda Build of Forklift Operator 下的 **已部署的 Operator → 资源实例**。
...
*注意：将自动创建一个名为 **host** 的提供者，以表示当前的 ACP 集群，仅作为目标。*
...
`spec.map` 是一个数组；每个条目描述一个 *源数据存储 → 目标 StorageClass* 映射。**每个** 包含您打算迁移的虚拟机磁盘的数据存储必须有一个对应的条目，否则计划验证阶段将失败。
...
如果源 VMware 环境暴露多个数据存储，并且您要迁移的虚拟机的磁盘分布在其中多个数据存储上（例如，操作系统磁盘在 `datastore-A` 上，数据磁盘在 `datastore-B` 上），则为每个源数据存储在 `spec.map` 中添加一个条目：

- 每个源数据存储（通过其 UUID/moRef 识别）必须在 `spec.map` 中 **仅出现一次**。
- 多个源数据存储可以映射到 **不同** 的 StorageClasses，或者它们都可以映射到 **相同** 的 StorageClass。
- 建议在 `spec.map` 中覆盖 **每个** 虚拟机使用的数据存储；仅列出子集将导致计划验证失败。

以下示例将两个 VMware 数据存储映射到两个不同的 StorageClasses：
...
要将来自多个源数据存储的磁盘合并到单个目标 StorageClass 中，请在每个条目上设置相同的 `destination.storageClass` 值：
...
迁移后，给 PVC 打标签，以确保它们在 ACP UI 中正确关联到虚拟机并得到妥善管理。
...
一旦打上标签，虚拟磁盘将在 ACP 的虚拟机详细信息页面中可见。

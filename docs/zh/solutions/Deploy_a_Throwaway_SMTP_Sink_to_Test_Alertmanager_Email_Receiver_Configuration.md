...
ProductsVersion:
  - '4.1.0,4.2.x'
id: KB260500028
sourceSHA: fbdc7325fae4a02b01fbc6a0c4b059456eb31b45bf6b3571c65213bf68138189
---
...
在配置 Alertmanager 邮件接收器（`smtp_smarthost`、`smtp_auth_username`、`smtp_from`、`smtp_require_tls` 等）时，实际的交付路径会经过公司中继、反垃圾邮件过滤器和 TLS 链，这些可能会隔离、静默丢弃或限速测试告警。在这些层中的任何一个交付失败都很难判断配置错误是在 Alertmanager 中、在中继中，还是在接收者邮箱中。一个临时的集群内 SMTP 接收器可以让操作员在将 smarthost 切换回生产中继之前，验证 Alertmanager 管道的端到端功能。

## 解决方案

### 步骤 1 — 在集群内部署一个临时 SMTP 接收器

[`mailhog`](https://github.com/mailhog/MailHog) 是一个单二进制 SMTP 服务器，它将每条消息接受到内存存储中，并通过 HTTP UI 进行展示。将其作为一个由 ClusterIP 服务前置的单个 Pod 运行：
...
如果 Alertmanager 是由 Prometheus Operator 管理的，相应的 `AlertmanagerConfig` CR 使用相同的 `email_configs` 结构；如果是通过名为 `kube-prometheus-alertmanager`（或类似名称）的 `Secret` 配置的，请编辑该 Secret 的 `alertmanager.yaml` 负载并重启 Alertmanager Pods。
...
打开 `http://127.0.0.1:8025`，确认测试告警出现，并带有配置的 `From:` 和 `To:` 头部以及渲染的 Alertmanager 正文。相同的数据也可以通过 JSON API 获取：
...
- 如果上游强制使用 TLS，请仅在烟雾测试中设置 `smtp_require_tls: false` — 在指向生产中继之前重新启用它。

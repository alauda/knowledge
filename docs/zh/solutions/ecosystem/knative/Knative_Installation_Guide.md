...
ProductsVersion:
  - '4.1,4.2,4.3,4.4'
id: KB260700063
sourceSHA: f4708246474c15d736594f591d769a32fb27df42e05579b6f9dc89a6ae6c8b09
---
...
**Alauda 对 Knative 的支持** 是 Alauda 应用服务 (S2，认证) 对上游 CNCF Knative Operator 的打包，列在 Alauda Cloud 市场上，并可从 ACP OperatorHub 安装。

Knative 是一个 CNCF 项目，为 Kubernetes 添加无服务器构建块。它有两个组件：

- **Serving** — 运行无状态、请求驱动的工作负载，支持零规模、基于修订版的流量分配和可插拔的入口层 (Kourier / Istio / Contour)。
- **Eventing** — 通过 Brokers、Triggers、Channels 和 Sources 传递 CloudEvents。

在 Alauda 容器平台 (ACP) 上，Knative 作为 OLM Operator 提供，您可以从市场上安装。该 Operator 管理 `KnativeServing` 和 `KnativeEventing` 自定义资源的生命周期。本指南描述了如何从 ACP 市场安装 **Alauda 对 Knative 的支持**，启动 Knative Serving 并使用 Kourier 入口，验证无服务器 `Service` 的端到端功能。
...
- 集群的 OperatorHub 中可用 **Alauda 对 Knative 的支持** 插件。如果尚未上传，管理员可以使用 `violet` CLI 推送：
  ```bash
...
预期：CSV `alauda-support-for-knative.v<version>` 达到 `Succeeded` 阶段，`knative-operator` 部署（操作员自己的部署名称，与上游保持不变）显示 `1/1` 准备就绪。
...
- **必须设置 `spec.registry.override`** 以将数据平面镜像重写为 *tag* 形式。Operator 的嵌入清单通过摘要引用操作数，而平台镜像白名单无法重写摘要引用 — 因此在隔离集群中，Pods 将无法拉取。`queue-proxy` 侧车来自 `config-deployment`，单独设置。
...
spec:
  registry:
    override:
      # factory:auto:install-images BEGIN  (操作数标签 == 操作员的嵌入服务版本)
...
- **此版本不支持单栈 IPv6 / IPv6-primary 双栈集群。** Serving `autoscaler` 进入 `CrashLoopBackOff`：上游的 stat-forwarder 硬编码了 bucket-lease `EndpointSlice` `AddressType` 为 IPv4 (`pkg/autoscaler/statforwarder/leases.go`)，因此在 IPv6 Pod IP 上，API 服务器拒绝它 (`endpoints[0].addresses ... must be an IPv4 address`)，`KnativeServing` 永远不会变为 `Ready`。上游已在 `main` 中修复 ([knative/serving#16591](https://github.com/knative/serving/pull/16591))，但尚未包含在发布的 1.22.x 中。此插件遵循社区发布流，因此一旦发布包含修复的 Knative 版本，限制将消除。在此之前，请使用 IPv4 或 IPv4-primary 双栈集群。
...
**问：`autoscaler` Pod 处于 `CrashLoopBackOff` 状态，`KnativeServing` 从未变为 Ready。**
...
**问：Serving Pods 卡在 `ImagePullBackOff`。**

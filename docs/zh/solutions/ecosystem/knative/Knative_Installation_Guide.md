...
ProductsVersion:
  - '4.1,4.2,4.3'
id: KB260700063
sourceSHA: 12152bebf0c3aff019f5b62a54d6adfe380acca7444e5b4aa0fcc7cd743063ae
---
...
**Alauda 对 Knative 的支持** 是 Alauda 应用服务 (S2，认证) 打包的上游 CNCF Knative Operator，列在 Alauda Cloud 市场上并可从 ACP OperatorHub 安装。

Knative 是一个 CNCF 项目，为 Kubernetes 添加无服务器构建块。它有两个组件：

- **Serving** — 运行无状态、请求驱动的工作负载，支持零扩展、基于修订的流量分配和可插拔的入口层 (Kourier / Istio / Contour)。
- **Eventing** — 通过 Brokers、Triggers、Channels 和 Sources 传递 CloudEvents。

在 Alauda Container Platform (ACP) 上，Knative 作为 OLM Operator 提供，您可以从市场安装。该 Operator 管理 `KnativeServing` 和 `KnativeEventing` 自定义资源的生命周期。本指南描述了如何从 ACP 市场安装 **Alauda 对 Knative 的支持**，启动 Knative Serving 并使用 Kourier 入口，验证无服务器 `Service` 的端到端功能。
...
- 在您集群的 OperatorHub 中可用的 **Alauda 对 Knative 的支持** 插件。如果尚未上传，管理员可以使用 `violet` CLI 推送：
  ```bash
...
预期：CSV `alauda-support-for-knative.v<version>` 达到 `Succeeded` 阶段，`knative-operator` 部署（操作员自己的部署名称，与上游保持不变）显示 `1/1` 就绪。

## 快速开始：使用 Knative Serving 部署无服务器服务

### 1. 创建 KnativeServing 实例

Knative Serving 是集群单例。适用两个 ACP 特定规则：

- **使用 Kourier 时必须在 `knative-serving` 命名空间中创建** — Operator 的 kourier-bootstrap ConfigMap 硬编码了 xDS 地址 `net-kourier-controller.knative-serving`。
- **您必须设置 `spec.registry.override`** 以将数据平面镜像重写为 *tag* 形式。Operator 的嵌入清单通过摘要引用操作数，而平台镜像白名单无法重写摘要引用 — 因此在气隙集群中，Pod 将无法拉取。`queue-proxy` 侧车来自 `config-deployment`，并单独设置。
...
预期：`Ready` 为 `True`，核心部署 (`activator`, `autoscaler`, `controller`, `webhook`, `net-kourier-controller`, `3scale-kourier-gateway`) 全部运行中。
...
# READY 应为 True，且 URL 被填充，例如 http://hello.default.<domain>
...
在 `knative-eventing` 命名空间中创建 `KnativeEventing` 实例，将相同的 `spec.registry.override` 模式应用于事件处理镜像（控制器、webhook、broker 过滤器/入口、内存通道、mtping、jobsink），并固定到操作数版本。有关 Broker/Trigger 配置，请参见 [上游事件处理文档](https://knative.dev/docs/eventing/)。
...
- 此版本验证的安装路径涵盖 **使用 Kourier 的 Serving**；事件处理和非 Kourier 入口 (Istio / Contour) 由用户根据上游文档安装。
...
# 从管理员 > 市场 > OperatorHub 卸载 Operator（或删除其 Subscription/CSV）
...
**问：`autoscaler` Pod 处于 `CrashLoopBackOff` 状态，`KnativeServing` 从未变为就绪。**
检查集群的 IP 家族：`kubectl get pod <autoscaler-pod> -n knative-serving -o jsonpath='{.status.podIPs}'`。如果 Pod IP 是 IPv6，您遇到了上述单栈 IPv6 的限制 — 请使用 IPv4 或 IPv4-primary 双栈集群，直到包含上游修复的 Knative 版本可用。

**问：Serving Pods 被卡在 `ImagePullBackOff`。**
确保 `KnativeServing` CR 上存在 `spec.registry.override`（以及 `config.deployment` 下的 `queue-sidecar-image`）。如果没有，Operator 将通过摘要部署操作数，而平台镜像白名单无法重写，因此气隙集群无法拉取它们。
...
**问：如何升级 Knative？**
从市场将 Operator 升级到新版本；它会将 `KnativeServing` / `KnativeEventing` 与匹配的操作数版本进行协调。将 `spec.registry.override` 中的镜像标签更新为新操作数版本以保持固定。

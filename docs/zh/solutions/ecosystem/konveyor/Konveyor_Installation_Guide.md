...
ProductsVersion:
  - '4.1,4.2,4.3,4.4'
id: KB260700098
sourceSHA: 84e167afcc53243bac4ace9dec9b99b41c84cc75d9d281771d4a770b4aab8532
---
...
Konveyor 是一个 CNCF 项目，帮助团队将现有应用迁移到 Kubernetes。它可以让您：

- **清点和评估** 应用 — 记录其业务服务、所有者和依赖关系，然后回答评估问卷以揭示风险。
- **分析源代码**，支持 Java、Python、Node.js 和 C#，以查找迁移障碍，并对每个发现进行工作量估算。
- **发现应用实际使用的技术**。
- **规划迁移波次** 并导出报告。

在 Alauda Container Platform (ACP) 上，该平台作为一个 Operator 提供，您可以从市场中安装。创建一个单一的 `Tackle` 资源即可启动整个平台 — 一个 REST API（hub）、一个 Web 控制台（ui）以及分析附加组件，并在此后保持其一致性。
...
| 网络                                | IPv4, IPv6             |
| Alauda 对 Konveyor 的支持 (包)       | v0.9.2                 |
| Konveyor 平台                       | v0.9.2                 |
| 许可证                              | Apache-2.0             |
...
- 集群的 OperatorHub 中可用的 **Alauda 对 Konveyor 的支持** 插件。如果尚未上传，管理员可以使用 `violet` CLI 推送它：
  ```bash
  ...
  ```
- 针对目标集群配置的 `kubectl`。
- **支持 ReadWriteOnce 卷的 StorageClass。** 平台请求两个卷，如果没有一个则无法启动。如果您的集群没有将 StorageClass 标记为默认，您必须在 `Tackle` 资源中显式命名它 — 请参见 [首先规划存储](#plan-storage-first)。

## 首先规划存储

这是安装看似挂起的最常见原因，因此在安装之前决定它。
...
1. **如果您从控制台表单创建 `Tackle` 资源**，它将预填充上述较小的大小，大多数集群可以满足。
2. **如果您手动编写 YAML 并省略大小字段**，您将获得上游默认值 — 包括一个 **100Gi** 的文件存储，许多集群无法绑定。始终显式设置 `hub_database_volume_size` 和 `hub_bucket_volume_size`。

将 `rwo_storage_class` 和 `hub_bucket_storage_class` 设置为真实的 StorageClass 名称，除非您的集群有默认值。检查：
...
2. 保持默认通道（`alpha`）。对于 **安装位置**，保持建议的命名空间 **`konveyor-tackle`** — 平台设计为在与 Operator 相同的命名空间中运行。
3. 确认安装。
...
预期：条目 `alauda-support-for-konveyor.v<version>` 达到 `Succeeded` 阶段，Operator 自身的 Deployment 显示 `1/1` 就绪。

## 快速开始：启动平台

### 1. 创建 Tackle 资源

从控制台打开已安装的 Operator 并创建一个 **Tackle** 实例 — 表单已预填充有效值。要通过命令行执行此操作：
...
> `Tackle` 直接接受平台的任何配置键在 `spec` 下。如果您使用 `kubectl apply` 并使用 CRD 未列出的键，请添加 `--validate=false`。
...
> **首次启动需要几分钟。** hub 没有就绪探针，因此在准备数据库完成之前，它会报告可用。请等待 API 响应（下一步），而不是仅仅信任 Deployment 状态。
...
预期：API 返回 `200`，分析器 / 发现 / 平台附加组件和语言提供程序扩展已注册。
...
- **在此版本中无法启用身份验证。** 保持 `feature_auth_required: "false"`。启用它会启动一个 Keycloak 服务器及其数据库，其容器镜像仅发布为 amd64，并未包含在此包中 — 它们将在 arm64 集群上无法启动，并且在没有互联网访问的集群上根本不可用。控制台可以在没有登录的情况下访问；如果需要，请在网络层（Ingress 规则，NetworkPolicy）限制访问。
- **实验性的 AI 辅助迁移功能不可用。** 出于同样的原因，保持 `experimental_deploy_kai` 未设置。
...
- **运行端到端源分析不在验证的安装路径中。** 此版本验证平台的安装、提供 API 和控制台、注册附加组件，并在数据完整的情况下存活重启。分析真实应用还需要可访问的源代码库和足够的备用容量以容纳分析 Pod。
- **没有从旧的社区 `konveyor-operator` 目录条目 (0.6.0-beta.1) 的升级路径。** 这是一个单独的新发布包。请全新安装，不要期望旧条目升级到此版本。
...
删除 `Tackle` 资源将移除 hub、控制台和 **两个卷**，因此请先备份您需要的任何内容。然后从 **管理员 > 市场 > OperatorHub** 卸载 Operator，或者：
...
**问：卷声明保持 `Pending`，没有任何启动。**
可能是集群没有默认的 StorageClass，或者请求的大小无法满足。运行 `kubectl -n konveyor-tackle describe pvc` 查看是哪种情况。将 `rwo_storage_class` 和 `hub_bucket_storage_class` 设置为真实类名，并检查 `hub_bucket_volume_size` — 手动编写的 YAML 如果省略它会请求 100Gi。
...
**问：我该如何升级？**
从市场中升级 Operator。它会将平台组件滚动到匹配的版本，并保留现有卷。

...
ProductsVersion:
  - '4.1,4.2,4.3'
id: KB260700104
sourceSHA: ca67e6fbe37a6642c7aa1ab60b7526e7b77ee042e41568f7768f265975c63a4c
---
...
**Alauda 对 lakeFS 的支持** 是 Alauda 应用服务 (S2，认证) 打包的上游 [lakeFS](https://lakefs.io/) 数据版本控制系统，列在 Alauda Cloud 市场上，并可以从 ACP OperatorHub 安装。

lakeFS 为您的对象存储提供了 Git 的工作流。它位于您已经使用的存储前面，让您可以：

- **分支** 您的数据 — 立即创建整个数据集的隔离副本，而无需复制数据。
- **提交** 更改并保留完整的、可审计的历史记录，记录谁在何时更改了什么。
...
在 Alauda 容器平台 (ACP) 上，lakeFS 作为一个 Operator 提供，您可以从市场中安装。创建一个 `LakeFS` 资源后，服务将启动并保持一致。
...
- 在您集群的 OperatorHub 中可用的 **Alauda 对 lakeFS 的支持** 插件。如果尚未上传，管理员可以使用 `violet` CLI 推送它：
  ...
  ```
- 针对目标集群配置的 `kubectl`。
- **关于 lakeFS 存储数据的位置的决定** — 请参见下一节。在安装之前做出决定；稍后更改意味着移动数据。

## 在安装之前决定存储

lakeFS 保留两种不同的内容，并且它们是单独配置的：

|                    | 内容                                                                  | 存储位置                                                 |
| ------------------ | --------------------------------------------------------------------- | -------------------------------------------------------- |
| **元数据存储**    | 提交图 — 分支、提交以及每个提交指向的对象                             | 嵌入式键值存储在磁盘上，**或** PostgreSQL                |
| **块存储**        | 对象数据本身                                                          | 磁盘上的目录，**或** S3 兼容对象存储                     |

该包的默认设置为 **local**（嵌入式存储，本地目录）。这让您可以一键启动一个工作中的 lakeFS，并且这是评估的正确选择 — 但请注意两个后果：

1. **它仅为单副本。** 嵌入式元数据存储无法共享，因此 `replicaCount` 必须保持为 `1`。
2. **本地意味着容器文件系统，除非您附加了一个卷。** 如果没有卷，当 pod 重启时，您的存储库及其历史记录将会丢失。
...
| 评估，持久         | 嵌入式，使用 PVC            | 在同一 PVC 上           | 1      | 演示和必须在重启后存活的小型试验              |
| **生产**           | **PostgreSQL**               | **S3 兼容存储**        | 2+     | 实际工作负载                                   |

后两者的详细信息如下。

该包配置的所有三种本地路径都位于一个目录下，因此一个 **挂载在 `/home/lakefs` 的单个卷** 可以持久化所有内容：

```
/home/lakefs/metadata   嵌入式元数据键值存储
/home/lakefs/data       对象数据
/home/lakefs/cache      已提交数据缓存
```
...
预期：条目 `lakefs-operator.v<version>` 达到 `Succeeded` 状态，Operator 自身的 Deployment 显示 `1/1` 准备就绪。

## 快速开始：启动 lakeFS

### 1. 选择加密密钥

`secrets.authEncryptSecretKey` 是 **必需的**。lakeFS 使用它来加密它存储的凭证，因此：

- 使用 **长随机字符串** — 例如 `openssl rand -hex 32`；
- **之后绝不要更改它。** 旋转密钥会使每个已存储的凭证无法解密，从而锁定您自己安装的访问。
...
spec:
  ...
  # storageClassName: <your-storage-class>   # 仅在集群未标记为默认时省略
  resources:
    requests:
      storage: 20Gi
---
...
对于 **评估** 部署 — 使用已提供的嵌入式元数据存储，您可以通过在第一次应用资源之前在 `lakefsConfig` 中放置一个 `installation` 块来无须干预地完成此步骤：
...
> **当元数据存储为 PostgreSQL 时，这不起作用。** 使用 `database.type: postgres`，服务器正常启动但保持未初始化，所有 API 调用都被拒绝，提示 `credentials not found`。通过上述设置页面设置 PostgreSQL 支持的部署，或在 pod 内部运行设置命令：
>
> ```bash
> kubectl -n lakefs exec deploy/lakefs -- /app/lakefs setup --user-name admin
> ```
>
> 该命令打印出它创建的访问密钥 ID 和秘密访问密钥 — 复制它们，因为它们不会再次显示。它生成自己的密钥对并忽略您设置的任何 `installation` 值。

### 5. 尝试 Git 类似的工作流

`lakectl` 随镜像一起提供，因此您可以在不安装任何东西的情况下运行整个回合：
...
预期：`f1.txt` 和 `f2.txt` 都在 `main` 上列出，日志显示合并。
...
首先创建数据库和存储桶。所有 `secrets.*` 值都存储在 Kubernetes Secret 中，而不是 ConfigMap 中。

您在此部署中创建的存储库必须使用 `s3://` 存储命名空间，例如 `lakectl repo create lakefs://demo s3://my-bucket/demo`。

**在首次使用之前进行设置。** PostgreSQL 支持的部署在未初始化状态下启动，并会拒绝每个请求，直到您完成设置 — 通过设置页面，或使用 `kubectl -n lakefs exec deploy/lakefs -- /app/lakefs setup --user-name admin`。请参见 [完成首次设置](#4-complete-the-first-time-setup) 中的说明。
...
| 字段                                | 含义                                                                                                                  |
| ------------------------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `secrets.authEncryptSecretKey`       | **必需。** 加密存储的凭证。永远不要更改它。                                                                          |
...
| `replicaCount`                       | 保持为 `1`，除非元数据存储为 PostgreSQL。                                                                           |
...
| `ingress.enabled`, `ingress.hosts`   | 对控制台、API 和 S3 网关的外部访问。                                                                                  |
| `resources`                          | CPU 和内存请求及限制。                                                                                              |

该包的两个行为与上游 chart 默认值不同，故意为之：

- **Pod 以非特权用户身份运行** — 作为非根用户，所有 Linux 功能被丢弃，并应用默认的 seccomp 配置文件。如果您附加了卷，它的 `fsGroup` 会为您设置，以便 lakeFS 可以写入。
- **关闭出站遥测和版本检查**，因此部署不会调用互联网，并在隔离集群上保持不变。

## 已知限制

<!-- factory:auto:known-limitations BEGIN -->

- **出厂默认设置为单副本。** 嵌入式元数据存储无法在 pod 之间共享，因此 `replicaCount` 必须保持为 `1`，直到您将元数据存储移动到 PostgreSQL。在此之前扩展将产生不一致的结果。
- **开箱即用的数据不持久。** 如果没有附加卷，本地元数据存储和块存储位于容器文件系统上，并在重启时丢失。在 `/home/lakefs` 附加 PVC（评估）或使用 PostgreSQL + S3（生产）。
- **无法旋转加密密钥。** `secrets.authEncryptSecretKey` 加密存储的凭证；在现有安装中更改它会使凭证无法解密。选择一次并保留副本。
- **没有从旧的 `lakeFS` chart 插件的升级路径。** 这是一个单独的新发布 Operator 包。请全新安装；现有的基于 chart 的条目不会升级到此。要跨越数据，请将新部署指向相同的 PostgreSQL 数据库和 S3 存储桶。
- **不包括 lakeFS 企业功能。** 此包是 Apache-2.0 开源 lakeFS。企业专用功能（超出内置模型的 RBAC、SSO、审计日志）不可用。
- **无干预设置在 PostgreSQL 元数据存储中不起作用。** `lakefsConfig` 中的 `installation` 块仅初始化评估部署。使用 `database.type: postgres` 时，部署启动但保持未初始化状态，直到您通过设置页面或使用 `/app/lakefs setup` 完成设置。请参见 [完成首次设置](#4-complete-the-first-time-setup) 中的说明。
...
工作负载被移除。您自己创建的 PVC **不会** 被删除 — 一旦您确认不再需要数据，请单独删除它：
...
> 在卸载 Operator 之前 **删除 `LakeFS` 资源**。Operator 是移除底层发布的工具；如果它先消失，资源无法完成删除，其命名空间将挂起在 `Terminating` 状态。
...
**问：我重启了 Pod，我的存储库消失了。**
没有附加卷，因此本地元数据存储位于容器文件系统上。按照快速开始的方式重新安装，挂载 PVC 到 `/home/lakefs`。

**问：我可以运行多个副本吗？**
只有在 PostgreSQL 作为元数据存储时可以。请参见 [生产配置](#production-configuration)。

**问：我丢失了设置页面上的秘密访问密钥。**
使用管理员帐户登录控制台并创建新的访问密钥对。原始秘密仅显示一次，无法恢复。

**问：现有的 S3 工具可以读取 lakeFS 存储库吗？**
可以。lakeFS 在同一端点上公开了一个 S3 兼容网关。将工具指向 lakeFS 服务地址，使用 lakeFS 访问密钥对，并将对象地址为 `s3://<repository>/<branch>/<path>`。

**问：我该如何升级？**
从市场升级 Operator。它会将 lakeFS Deployment 升级到匹配的版本，并保持您的元数据存储和块存储不变。

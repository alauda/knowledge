...
ProductsVersion:
  - 4.x
id: KB251200004
sourceSHA: 3ae2924f180a43b5ce5ed2f48323fd5cecc9b40038f4a701fd34d002234c66b0
---
...
本解决方案描述了如何基于 Ceph 块存储的灾难恢复能力构建 Nexus 灾难恢复解决方案。该解决方案实现了 **热数据，冷计算** 架构，其中数据通过 Ceph 块存储灾难恢复机制持续同步到备用集群。当主集群发生故障时，部署备用 Nexus 实例，备用 Nexus 将快速开始使用灾难恢复数据并提供服务。该解决方案主要关注数据灾难恢复处理，用户需要实现自己的 Nexus 访问地址切换机制。
...
| 术语                               | 描述                                                                                                                                                                   |
| ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **主 Nexus**                        | 处理正常业务操作和用户请求的活动 Nexus 实例。该实例完全运行，所有组件正常工作。                                                                                       |
| **备用 Nexus**                     | 计划在不同集群/地域中部署的备用 Nexus 实例，保持待命状态，直到在灾难恢复场景中激活。                                                                                  |
| **主块存储**                       | 存储所有 Nexus 数据的活动块存储系统，作为块存储复制的源。                                                                                                          |
| **备用块存储**                     | 从主块存储接收数据复制的同步备份块存储系统。在灾难恢复期间确保数据可用性。                                                                                          |
| **恢复点目标 (RPO)**              | 在时间上可接受的最大数据丢失量（例如，5分钟，1小时）。它定义了在灾难发生时可以丢失多少数据，超过这个量就变得不可接受。                                                |
| **恢复时间目标 (RTO)**            | 在时间上可接受的最大停机时间（例如，15分钟，2小时）。它定义了系统在灾难后必须多快恢复。                                                                              |
| **故障切换**                       | 当主系统不可用或发生故障时，从主系统切换到备用系统的过程。                                                                                                          |
| **数据同步**                       | 将数据从主系统持续复制到备用系统的过程，以保持一致性并启用灾难恢复。                                                                                                |
| **热数据，冷计算**                 | 一种架构模式，其中数据持续同步（热），而计算资源保持非活动状态（冷），直到故障切换发生。                                                                              |

## 架构

Nexus 灾难恢复解决方案为 Nexus 服务实现了 **热数据，冷计算架构**。该架构通过近实时数据同步和手动 Nexus 服务故障切换程序提供灾难恢复能力。该架构由两个部署在不同集群或地域的 Nexus 实例组成，备用 Nexus 实例在灾难场景中激活之前不会提前部署，而存储层保持持续同步。
...
使用 `Alauda Build of Rook-Ceph` 构建块存储灾难恢复集群。请参考 [块存储灾难恢复](https://docs.alauda.io/container_platform/4.1/storage/storagesystem_ceph/how_to/disaster_recovery/dr_block.html) 文档构建灾难恢复集群。

### 设置主 Nexus

按照 Nexus 实例部署指南部署主 Nexus 实例。配置域访问，使用主块存储进行数据存储。

配置示例（仅包含与灾难恢复相关的配置项，完整配置项请参见产品文档）：
...
在部署主 Nexus 后，您需要为 Nexus 组件使用的 PVC 配置 RBD Mirror。配置完成后，PVC 数据将定期同步到备用 Ceph 集群。有关具体参数配置，请参考 [Ceph RBD Mirror](https://docs.alauda.io/container_platform/4.1/storage/storagesystem_ceph/how_to/disaster_recovery/dr_block.html#enable-mirror-for-pvc)。
...
检查 Ceph RBD Mirror 状态，以查看 Nexus PVC 是否已配置为 Ceph RBD Mirror。
...
如果您需要验证备用集群中的 Nexus 是否可以成功部署，您可以暂时将备用集群的 Ceph RBD 提升为主集群，测试完成后再将其设置回备用状态。同时，您需要删除测试期间创建的所有 Nexus、PV 和 PVC 资源。
:::
...
获取主 Nexus 使用的密码 Secret YAML，并在备用集群中创建相同命名空间名称的 Secret。
...
修改备份的 PV 文件，删除 YAML 中所有 `spec.claimRef` 字段。

在灾难恢复环境中直接使用相同命名空间名称创建备份的 PVC 和 PV YAML 文件。
...
:::warning
`Nexus` 资源 **不需要** 在灾难恢复环境中立即创建。仅在发生灾难并执行灾难恢复切换时，才需要在备用集群中创建。
:::
...
RPO 代表在灾难恢复场景中可接受的最大数据丢失。在此 Nexus 灾难恢复解决方案中：

- **存储层**：由于 Ceph RBD 块存储复制 Nexus 数据，通过定期快照同步，数据丢失取决于同步间隔，可以 [配置](https://docs.alauda.io/container_platform/4.1/storage/storagesystem_ceph/how_to/disaster_recovery/dr_block.html#create-volumereplicationclass)
...
- Ceph RBD 故障切换：1-2 分钟（手动）
- Nexus 服务激活：2-5 分钟（手动）
- 外部路由更新：1-3 分钟（手动，取决于 DNS 传播）

## 灾难切换

1. **确认主 Nexus 故障**：确认所有主 Nexus 组件处于非工作状态，否则首先停止所有主 Nexus 组件。

2. **提升备用 Ceph RBD**：将备用 Ceph RBD 提升为主 Ceph RBD。请参考 [Alauda Build of Rook-Ceph 故障切换](https://docs.alauda.io/container_platform/4.1/storage/storagesystem_ceph/how_to/disaster_recovery/dr_block.html#procedures-1) 中的切换程序。

...
ProductsVersion:
  - '4.1.0,4.2.x'
sourceSHA: b72d70ff3ea94ad1b77615913188c7a1e1a9c5f52c9d051327fea13abe46cdcd
---

## 问题

在 ACP 备份表面（`configure/backup`，它打包了上游的 Velero 项目）上进行备份或恢复时，`spec.includedNamespaces` 中声明了一个通配符模式：
...
期望 Velero 会将该模式扩展为所有名称以 `test` 结尾的命名空间，并备份它们。然而，实际情况是 Velero 将字符串 `*test` 视为字面命名空间名称。由于没有命名空间的名称确实为 `*test`，备份记录在其元数据中按原样记录命名空间选择器，但从未找到任何工作负载对象进行捕获。备份文件的内容几乎为空——任何意外匹配的命名空间的命名空间定义可能会被写入，但内部的资源则没有。

此情况下的 Velero 日志显示模式逐字存储，随后针对该不可能的名称进行逐资源枚举：
...
这是 Velero 中一个长期存在的设计选择，而不是 ACP 打包中的缺陷。上游文档直接指出：*“包含/排除的命名空间必须单独列出；不支持通配符和正则表达式。”*

输出字段 `spec.resources`（要包含的对象类型）以相同方式支持单个 `*`，但其他情况下需要显式的 GVK 列表。同样适用“无正则表达式”规则。
...
这是最简单的修复，也是唯一在上游生存而不需要任何 Velero 侧更改的修复。

### 选项 B — 在命名空间上使用标签选择器

Velero 在 Backup CR 上尊重 `labelSelector` 并将其应用于 **所有对象** 在包含的命名空间中，这几乎总是与预期不符（它还会过滤掉未标记的 ConfigMaps 和 Deployments）。对于命名空间级别的过滤，建议对命名空间进行标记并在驱动程序处生成列表：
...
备份在创建 CR 时对命名空间标签成员资格是声明性的。将新命名空间添加到未来的备份集是一个标记操作；无需代码更改或 CR 编辑。
...
与 `excludedNamespaces` 配对，以排除 kube-system 级别的命名空间，这些命名空间在恢复过程中不会回传：
...
`Schedule` CR 根据 cron 生成备份，模板可以通过控制器或小型 CronJob 定期重新生成，该控制器或 CronJob 重新读取命名空间标签选择器并重写 Schedule 的模板。这保持了扩展的命名空间列表的新鲜感，而无需在每次备份时预计算。
...
在重写使用通配符的现有 `Schedule` 时，还需重新检查可能已预先编写的任何 `Restore` CR，这些 CR 可能针对相同的通配符——恢复侧同样是字面意义的，将找不到任何可恢复的内容：

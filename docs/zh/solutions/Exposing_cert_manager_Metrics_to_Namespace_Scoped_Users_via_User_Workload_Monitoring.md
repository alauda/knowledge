...
ProductsVersion:
  - '4.1.0,4.2.x'
id: KB260500004
sourceSHA: 013dabfda9f55eb698711f05ac4a82ed66aef04c0fd684418367c4b217d25ccd
---

# 通过用户工作负载监控向命名空间范围用户公开 cert-manager 指标

## 问题

命名空间范围的用户仅被授予对单个项目的访问权限，无法查看 `certmanager_certificate_expiration_timestamp_seconds` 指标（或任何其他 cert-manager 指标），即使集群上已启用用户工作负载监控。对于集群监控用户，相同的查询是有效的。
...
用户工作负载监控（UWM）运行一个独立的 Prometheus，该 Prometheus 抓取由 `ServiceMonitor` 和 `PodMonitor` 资源选择的目标。UWM 抓取的指标对在监控所在命名空间具有 `monitoring-rules-view`（或等效权限）的用户可见。要将 cert-manager 指标引入 UWM 并使命名空间范围的用户能够访问，必须满足两个条件：
...
默认的 cert-manager 图表提供一个名为 `cert-manager` 的服务，端口为 `tcp-prometheus-servicemonitor`（端口 9402）。请根据您的安装确认名称——较旧的图表可能使用 `metrics`。
...
如果用户已经在命名空间中拥有 project-edit 或 project-admin 角色，平台通常会在该角色中包含监控读取权限，因此不需要额外的绑定；请使用 `kubectl auth can-i` 验证。
...
返回 200 响应且 `result` 数组非空确认路径有效。如果响应为空，请验证用户工作负载 Prometheus 是否实际抓取了目标——请参见下面的诊断步骤。

### 替代方案：集群范围读取权限（不推荐）

当命名空间范围的公开不可行时，授予用户 `cluster-monitoring-view` 权限将允许访问集群 Prometheus 收集的 *所有* 指标：

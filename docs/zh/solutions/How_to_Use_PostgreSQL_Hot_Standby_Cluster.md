...
ProductsVersion:
  - 4.x
id: KB251000009
sourceSHA: 78eb990c082c152d08f31a42c2798874eb9f1089c09e6bb937cc40fc64d0b266
---
...
现代应用程序要求其 PostgreSQL 数据库具备高可用性和灾难恢复能力。传统的备份解决方案通常涉及显著的停机时间和数据丢失。手动复制设置复杂，配置和维护困难。

### 解决方案

本指南提供了使用 Alauda 容器平台 (ACP) 设置 PostgreSQL 热备份集群的全面说明。该解决方案支持集群内部和跨集群的复制，能够实现：

- **最小数据丢失**：持续的流复制确保最小的数据丢失（通常最多几秒的数据）
...
| 场景                   | 推荐方法               | 章节参考                               |
| ---------------------- | ---------------------- | -------------------------------------- |
| **高可用性**          | 集群内部复制           | [集群内部设置](#intra-cluster-setup)  |
...
- **存储资源**：
  ...
  - 备用集群：与主集群相同的存储容量，以确保完整的数据复制。确保 **StorageClass 性能（IOPS/吞吐量）** 与主集群匹配，以防故障转移后性能下降。
  - 考虑未来增长并设置适当的 `max_slot_wal_keep_size`（建议最低 10GB）
- **网络资源**：
  - 集群内部：标准 Kubernetes 网络性能
...
在继续之前记录此初始拓扑和复制状态；它提供了后续切换和恢复检查的基线。
...
在两个 Kubernetes 集群之间的流量通过防火墙的环境中，在创建集群之前决定允许哪些端口。默认情况下，操作员允许 Kubernetes 自动分配 NodePorts，每当服务被重新创建时，都会分配一个新端口——这是您无法提前提交给防火墙变更请求的分配，并且在 [故障排除](#troubleshooting) 中描述的集群重建步骤中不会保留。

**防火墙必须允许哪些端口**

| 方向                           | 目标节点                                       | 需要打开的端口                              |
| ------------------------------ | ---------------------------------------------- | ------------------------------------------- |
| 备用集群 → 主集群            | 每个运行主集群 PostgreSQL pod 的节点          | 主集群的 **master** 服务 NodePort         |
| 主集群 → 备用集群            | 每个运行备用集群 PostgreSQL pod 的节点        | 备用集群的 **master** 服务 NodePort       |

有两个容易出错的点：

- **在每个可以托管 PostgreSQL pod 的节点上打开端口，而不仅仅是您在 `peerHost` 中输入的地址。** 操作员记录每个成员 pod 的主机 IP，并从该列表构建跨集群端点，因此可以连接到其中的任何一个。如果 pod 可以重新调度到其他节点，也要包括这些节点。
- **两个方向都是必需的。** 在复制正常运行时，只有备用集群拨打主集群的电话，但在切换期间方向会反转：降级的集群成为拨打电话的一方。仅允许备用 → 主的规则在第一次切换之前有效，然后就会失败。
...
| --------------------- | ------------------- | ---------------------- | -------------------------------- |
| `<cluster-name>`      | master              | `master`               | **是** — 这是需要打开的 NodePort |
| `<cluster-name>-repl` | replica             | `replica`              | 否                               |
| `<cluster-name>-xcr`  | 跨集群别名         | 不可配置               | 否                               |
| `<cluster-name>-exporter` | metrics         | 不可配置               | 否                               |

:::warning `-repl` 是副本服务，而不是复制服务
`-repl` 后缀代表 **副本**：这是一个只读客户端端点，在副本 pod 之间进行负载均衡，与跨集群复制无关。它与“复制”的相似性使其成为请求防火墙规则时最常见的错误答案。
:::

:::info `-xcr` 服务 NodePort 不需要打开
`<cluster-name>-xcr` 服务也会接收其自己的自动分配 NodePort，但它是 **peer** 集群的本地别名：其目标端口是对等方的主 NodePort。流量通过它离开，没有流量到达它，外部客户端也不会连接到它。它也是由操作员直接构建的，而不是从 `serviceTemplates` 中构建的，因此其端口无法固定——也不需要固定。
:::
...
`clusterReplication` 没有端口号字段。使用 `spec.serviceTemplates.master` 固定主服务的 NodePort，该字段合并到生成的服务中：
...
将相同的模式应用于两个集群，如果它们共享防火墙策略，则为每个选择不同的数字。备用集群的 `peerPort` 就是您在主集群上固定的数字，当服务被重新创建时，它不再改变。

:::warning 完整写出端口条目
模板的 `ports` 列 **替换** 生成的列表，而不是逐字段合并，因此部分条目不会继承其替换条目的默认值。

架构捕获了最坏的情况：`port` 是必需的，省略它的条目会被拒绝，错误信息为 `spec.serviceTemplates.master.spec.ports[0].port: Required value`。`targetPort` 是可选的，Kubernetes 默认将其设置为 `port` 的值。完整写出条目，如上所示，使结果独立于这两种行为，并使意图可读。
:::

:::warning 验证您的集群中的 CRD 是否接受 `serviceTemplates`
该字段在与操作员捆绑的 CRD 架构中存在，但并非每个打包的 CRD 副本中都存在。如果操作员以 `enable_crd_registration: true` 运行，它可以用省略该字段的架构替换已安装的 CRD。

这取决于客户端的表现。`kubectl apply` 默认请求严格字段验证，并会发出响亮的失败：

```
error: ... strict decoding error: unknown field "spec.serviceTemplates"
```

不请求严格验证的客户端——`kubectl apply --validate=ignore`、旧客户端以及一些控制器和 GitOps 工具——会被告知对象已成功创建，而 API 服务器会丢弃该字段。NodePort 然后保持自动分配，日志中没有解释原因。确认架构而不是依赖于应用成功：
...
:::warning 更改配对集群的节点端口会破坏复制，并且不会自行修复
对等方从 `sys_operator.multi_cluster_info` 元数据表中学习端口，该表通过复制流到达备用集群。因此，主集群的节点端口更改会切断流，更新的行永远不会到达：主集群记录新端口，而备用集群则无限期保留旧端口，将其 `-xcr` 端点指向不再存在的端口。两侧无法自行重新同步，因为唯一可以传递更正的通道是更改切断的通道。

恢复的方法是恢复先前的节点端口，此时备用集群重新连接并赶上，或者重建备用集群。提前规划数字，而不是在运行的配对上进行调整。
:::

**替代方案：使用负载均衡器**

使用 `replSvcType: LoadBalancer`（操作员 v4.2.0 或更高版本），对等方通过端口 5432 连接到负载均衡器地址，而不涉及 NodePort，这通常使防火墙规则更简单且稳定。这需要两个集群中都有负载均衡器实现。

:::info 在重新编号端口之前确认故障确实是防火墙
防火墙丢包和集群自身的覆盖网络故障在 PostgreSQL pod 内部看起来是相同的：连接超时。从备用侧的 **节点 shell** 测试，而不是从 pod 测试：

```bash
nc -vz <primary-node-ip> <primary-master-nodeport>
```

`Connection refused` 表示路径是开放的，但没有监听该端口——端口号或服务是错误的。超时与防火墙丢包一致。如果节点级测试成功，但 pod 级测试超时，则问题出在集群网络内部，更改端口号不会解决问题。
:::
...
为避免脑裂情况，在三个阶段中执行计划切换。**第 3 阶段不是可选的**——提升新主节点并不会改变应用程序发送数据库连接的地方。

> **重要**：对于跨集群设置，请确保在执行命令之前将 `kubectl` 上下文切换到适当的集群。

:::info 在切换之前确认复制元数据是干净的
第 1 阶段在 **原地** 降级当前主节点。降级的集群从 `sys_operator.multi_cluster_info` 表中获取新对等方的连接详细信息，因此切换的可靠性仅与该表的内容一样。
...
⚠ **切换不会为您执行此操作。** 在第 1 和第 2 阶段之后，前主节点是一个 **只读备用**，而应用程序的数据库连接仍然指向它。写入操作将继续失败：
...
健康的复制并不意味着服务已恢复——**应用程序连接必须显式重新指向。**
...
> 在跨集群设置中，这与配置备用时使用的 `peerHost` / `peerPort` 是同一类路径——确保防火墙也允许它。

**3. 确认凭证**

数据库凭证存储在名为 **集群名称** 的密钥中，因此密钥名称随集群而变化：
...
确认 `read-only transaction` 错误不再出现在应用程序或数据库日志中。
...
2. 更新应用程序连接以指向新主节点（见 *正常操作 → 第 3 阶段*）
...
两种不同的情况会产生相同的症状，并且它们有不同的补救措施。在采取行动之前，请确定您遇到的是哪一种。

- **备用落后，所需的 WAL 已被回收。** 备用日志重复 `requested WAL segment ... has already been removed`。数据是完整且一致的；备用只是无法获取所需的 WAL 以赶上。
- **备用已分歧。** 备用日志报告 `requested starting point ... is not in this server's history`，或者两个集群报告不同的 `system identifier` 值。流复制无法调和这一点。
...
# 3. 主节点是否仍持有备用所需的 WAL，并且是否有任何东西保留它？
...
使用主节点的 **当前领导者**，这不一定是 pod `-0`。使用 `patronictl list` 确定它。

##### 解决方案

:::warning 重建会放弃您以其他方式恢复的能力
下面的过程会删除备用集群。删除它会移除 `spec.clusterReplication`，因此操作员会在主节点上删除 `xdc_hotstandby` 复制槽，并且 **所有 WAL 保留对于该对等方立即释放**。备用所需的任何 WAL 在主节点的下一个检查点时变得可以回收。

如果备用仅落后而不是分歧，应用此过程会将可恢复的情况转换为需要完整基础备份的情况，并破坏确定复制停止原因所需的证据。

**在删除任何内容之前：**

1. 完成上述诊断步骤并记录输出。

2. 如果缺失的 WAL 仍然存在——在主节点的 `pg_wal` 中，在 WAL 存档中，或在主集群的其他成员中——可以通过使这些段对备用可用来修复备用。将完整段复制到备用的 `pg_wal` 目录中（由 `postgres` 拥有，模式为 `600`），然后重启它。通过校验和验证每个复制的文件与其源的匹配：一个卡住的备用通常已经持有它失败的段的 **部分** 复制，而该部分文件的大小为 16 MiB，因此文件大小并不能指示完整性。
...
**步骤 2 必须在步骤 3 之前完成，并且不能跳过。** 每个集群的行由从集群的 Kubernetes UID 派生的 ID 键入，因此重新创建的集群将在 *新* ID 下注册。如果旧行仍然存在，表中将持有同一集群的两行——一行当前，一行过时。请参见
[过时的复制元数据](#stale-replication-metadata-after-recreating-a-cluster)。
...
5. 在备用集群运行后，确认复制确实被保留——而不仅仅是连接。该槽必须存在，处于活动状态，并跟踪备用：
...
空的 `restart_lsn` 意味着该槽不保留任何内容，无论 `max_slot_wal_keep_size` 如何，WAL 都会被回收。缺少行意味着不存在槽，对等方根本没有保留。

#### 重新创建集群后的过时复制元数据

##### 症状

在切换后或重新创建集群后，复制未恢复，即使两个集群都在运行且它们之间的网络路径是开放的。降级或重新创建的集群可能在不再使用的端口或地址上寻址对等方。

##### 原因

跨集群复制将每个参与集群的连接详细信息存储在主节点的 `sys_operator.multi_cluster_info` 表中。每行的 ID 是从集群的 Kubernetes UID 派生的，因此 **重新创建的集群在新 ID 下注册，而不是更新现有行**。如果先前的行没有先被删除，表中将持有描述同一集群的两行，其中一行是过时的。

在 **PostgreSQL Operator 版本早于 v4.3.4** 的情况下，读取对等方详细信息的查询不应用任何排序，返回的第一行将被使用，因此可能会选择过时的行而不是当前行。这些版本在重新注册时也不会刷新行的 `last_update` 时间戳，因此时间戳无法可靠地指示哪一行是当前的。
...
期望每个集群恰好一行。对于同一 `cluster_name` 的多于一行表示存在剩余的元数据。将 `node_port` 与对等方当前主服务进行比较：
...
1. 删除过时的行，仅保留 `node_port` 和 `member_hosts` 与对等方当前主服务匹配的行：

```sql
DELETE FROM sys_operator.multi_cluster_info WHERE id = <obsolete-id>;
```

2. 允许操作员进行协调，然后确认 `-xcr` 服务端点现在寻址对等方的当前端口：
...
- 适当设置 `max_slot_wal_keep_size`（生产环境最低 10GB）
- 使用具有足够 IOPS 的专用存储类用于数据库工作负载
- 实施复制延迟和集群健康的监控
- 在非生产环境中定期测试故障转移程序
...
本指南提供了在 Alauda 容器平台上实施 PostgreSQL 热备份集群的全面说明。该解决方案通过流复制和手动故障转移管理提供企业级的高可用性和灾难恢复能力。

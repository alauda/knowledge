...
ProductsVersion:
  - '4.1.0,4.2.x'
id: KB260500003
sourceSHA: 3de7c35a9281e6690e2cd75536f11e2a95100d1986f62872998bb5d9f4b47fe1
---

## 问题

一个工作节点在加入集群后不久消失，或者在稳定状态下消失。`kubectl get nodes` 不再列出它，主机上的 kubelet 是健康的，并继续发送心跳调用（这会在同名下重新创建节点，循环重复），并且没有明显的控制器状态条件解释删除原因。

反复出现的问题是 *谁* 删除了节点对象。Kubernetes 审计日志记录每个 API 写入，包括发起请求的客户端身份。通过正确的查询，审计日志可以识别负责的服务帐户、控制器或人工用户。

## 根本原因

节点删除是一个常规的 API 调用：`DELETE /api/v1/nodes/<name>`。任何对 `nodes` 具有 `delete` 权限的实体都可以发出此请求。集群中通常的删除者有：

- 正在进行维护的集群操作员（通过 `kubectl` 的真实用户）。
- 集群自动扩缩器移除一个使用不足的节点。
...
在每个控制平面节点上运行结构化搜索 apiserver 审计日志，过滤出针对受影响节点资源的 `delete` 操作。以下查询返回每个匹配审计事件的一行，包含时间戳、动词、请求 URI、目标对象、调用身份和源 IP：
...
`user.username` 字段准确告诉您是谁发出了删除请求：在此示例中是来自 `cluster-policy` 命名空间的服务帐户 `policy-controller-sa`。从该标识符，您可以：

- 检查服务帐户所在的命名空间，以发现哪个控制器拥有它。
- `kubectl auth can-i delete nodes --as=system:serviceaccount:cluster-policy:policy-controller-sa` 确认该身份确实具有其使用的权限。
- 在相同时间窗口内读取控制器的日志，以查看 *为什么* 它决定删除该节点。

如果用户名是一个真实的人（例如 `alice@example.com`），则删除是一次手动操作；调查触发它的人为运行手册。

如果用户名属于您不认识的系统控制器，则该控制器是禁用、通过 RBAC 限制或重新配置以停止删除您希望保留的节点的候选者。
...
删除授予 `delete` 权限的绑定。控制器将开始记录权限错误，而不是静默删除节点——这是您需要的可见性，以便正确修复它。

## 诊断步骤

如果上述审计搜索返回零行，可能有两种故障模式：
...
1. **审计日志未启用。** 检查控制平面主机上的 apiserver 进程参数：
   ...
   `--audit-policy-file` 和 `--audit-log-path` 都应该设置。如果没有，请配置审计策略（记录所有对 `nodes` 的 `verbs` 的宽松策略足以进行此调查），并重新启动 apiserver pods。

2. **审计策略过滤掉了 `nodes` 上的 `delete` 动词。** 检查审计策略：
   ...
   为 `nodes` 资源添加一个 `level: Metadata`（或更高）规则，以便将来的删除被记录。
